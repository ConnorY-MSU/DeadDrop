#include "aes128.h"
#include <string.h>
#include <stdint.h>
#include <stddef.h>
 
#define AES_ROUNDS 10
#define ROUND_KEYS_SIZE ((AES_ROUNDS + 1) * AES128_KEY_SIZE)

static const uint8_t sbox[256] = {
    
};

static const uint8_t inv_sbox[256] = {
    
};

static const uint8_t rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};
/* Multiply by 2 in GF(2^8); 0x1B (FIPS 197 Sec. 4.2 reduction polynomial) is XORed in only on overflow of the top bit. */
static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ ((x & 0x80) ? 0x1B : 0x00));
}
void aes128_key_expansion(const uint8_t key[16], uint8_t round_key[ROUND_KEYS_SIZE]) {
    uint8_t temp[4];
    int bytes_generated = 16;
    int rcon_iter = 1;
 
    /* The first round key is just the original key. */
    memcpy(round_key, key, 16);
 
    while (bytes_generated < ROUND_KEYS_SIZE) {
        memcpy(temp, round_key + bytes_generated - 4, 4);
 
        if (bytes_generated % 16 == 0) {
            /*cyclic left-shift of the 4 bytes by one byte. */
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;
 
            /* apply the S-box to each byte. */
            for (int i = 0; i < 4; i++) temp[i] = sbox[temp[i]];
 
            /* XOR */
            temp[0] ^= rcon[rcon_iter++];
        }
 
        for (int i = 0; i < 4; i++) {
            round_key[bytes_generated] = round_key[bytes_generated - 16] ^ temp[i];
            bytes_generated++;
        }
    }
}
static void add_round_key(int round, uint8_t state[16], const uint8_t round_key[ROUND_KEYS_SIZE]) {
    for (int i = 0; i < 16; i++) state[i] ^= round_key[round * 16 + i];
}

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

static void inv_sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = inv_sbox[state[i]];
}
 
static void shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    tmp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = tmp;
    tmp = state[2];  state[2]  = state[10]; state[10] = tmp;
    tmp = state[6];  state[6]  = state[14]; state[14] = tmp;
    tmp = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7]  = state[3];
    state[3]  = tmp;
}
static void inv_shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    tmp = state[13];
    state[13] = state[9];
    state[9]  = state[5];
    state[5]  = state[1];
    state[1]  = tmp;

    tmp = state[2];  state[2]  = state[10]; state[10] = tmp;
    tmp = state[6];  state[6]  = state[14]; state[14] = tmp;

    tmp = state[3];
    state[3]  = state[7];
    state[7]  = state[11];
    state[11] = state[15];
    state[15] = tmp;
}
static void mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        uint8_t *col = state + 4 * c;
        uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
        col[0] = (uint8_t)(xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3);
        col[1] = (uint8_t)(a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3);
        col[2] = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3));
        col[3] = (uint8_t)((xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3));
    }
}

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return p;
}

/* InvMixColumns: multiply each column by the fixed GF(2^8) matrix (FIPS 197 Sec. 5.3.3), coefficients 0x0e/0x0b/0x0d/0x09. */
static void inv_mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        uint8_t *col = state + 4 * c;
        uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
        col[0] = (uint8_t)(gmul(a0,0x0e) ^ gmul(a1,0x0b) ^ gmul(a2,0x0d) ^ gmul(a3,0x09));
        col[1] = (uint8_t)(gmul(a0,0x09) ^ gmul(a1,0x0e) ^ gmul(a2,0x0b) ^ gmul(a3,0x0d));
        col[2] = (uint8_t)(gmul(a0,0x0d) ^ gmul(a1,0x09) ^ gmul(a2,0x0e) ^ gmul(a3,0x0b));
        col[3] = (uint8_t)(gmul(a0,0x0b) ^ gmul(a1,0x0d) ^ gmul(a2,0x09) ^ gmul(a3,0x0e));
    }
}

void aes128_encrypt_block(const uint8_t round_key[ROUND_KEYS_SIZE],
                          const uint8_t in[16], uint8_t out[16]) {
    uint8_t state[16];
    memcpy(state, in, 16);
 
    add_round_key(0, state, round_key);
    for (int round = 1; round < AES_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(round, state, round_key);
    }
    /* Final round has no MixColumns. */
    sub_bytes(state);
    shift_rows(state);
    add_round_key(AES_ROUNDS, state, round_key);
 
    memcpy(out, state, 16);
}

void aes128_decrypt_block(const uint8_t round_key[ROUND_KEYS_SIZE],
                          const uint8_t in[16], uint8_t out[16]) {
    uint8_t state[16];
    memcpy(state, in, 16);

    add_round_key(AES_ROUNDS, state, round_key);

    for (int round = AES_ROUNDS - 1; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(round, state, round_key);
        inv_mix_columns(state);
    }

    /* Final round has no InvMixColumns. */
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(0, state, round_key);

    memcpy(out, state, 16);
}


static void increment_counter(uint8_t counter[16]) {
    for (int i = 15; i >= 0; i--) {
        if (++counter[i] != 0) {
            break;
        }
    }
}

void aes128_ctr_xcrypt(const uint8_t round_key[ROUND_KEYS_SIZE],
                        const uint8_t initial_counter_block[16],
                        const uint8_t *input,
                        uint8_t *output,
                        size_t length) {
    uint8_t counter_block[16];
    uint8_t keystream_block[16];
    memcpy(counter_block, initial_counter_block, 16);

    size_t offset = 0;
    while (offset < length) {
        aes128_encrypt_block(round_key, counter_block, keystream_block);

        size_t chunk = (length - offset < 16) ? (length - offset) : 16;
        for (size_t i = 0; i < chunk; i++) {
            output[offset + i] = input[offset + i] ^ keystream_block[i];
        }

        increment_counter(counter_block);
        offset += chunk;
    }
}