...
Deployed and confirmed healthy (`NRestarts=0`, clean reconnect to peer) on `bravo`; `alpha` deployment in progress via the same `git pull` + rebuild + restart pattern used throughout this session.
