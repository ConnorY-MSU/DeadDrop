...
**Not individually tested with a real tap**: the lock-screen wake gesture and the WiFi-list tap-select, since both require keyboard input (Ctrl+L / Ctrl+W) to reach their respective states first, and no keyboard is attached yet. Both share the exact same proven touch-reading/dispatch mechanism the pause/resume gesture already validated end-to-end; code-reviewed but not yet individually confirmed with a physical tap. Revisit once keyboards arrive.
