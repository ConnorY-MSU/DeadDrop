...
### Not yet done - remaining audit scope

The originally-proposed audit plan had six areas; all six now have a real finding, a clean pass, or an honestly-assessed low-severity note documented above. Still open, for a future pass:
- Re-verify the four Week 2 Day 5 TLS/mTLS negative-path tests (no client cert, wrong CA, expired cert, revoked cert) still hold against the current build - written and verified against an earlier version of the codebase, never re-run since.
- Fuzz the protocol surfaces that never got the same treatment the original message parser did: the keyshare fetch protocol (`keyshare.c`), WiFi `nmcli`-output parsing (`wifi.c`), and the `/destroy` emergency-wipe command path.
- Consider persisting the PIN lock's rate-limiting state (Finding #6) to close even the low-severity gap, if/when there's a reason to believe the physical-access assumption alone isn't enough.
