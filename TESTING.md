...
Full, ongoing test log for `deaddrop`, started Week 1 Day 1 per project documentation standard. Entries are added incrementally as each component is built and validated — not rewritten after the fact.
...
**Distinct, deliberately different Common Names** for the CA (`DeadDrop Root CA`) vs. server (`DeadDrop-Server`) vs. client (`DeadDrop-Client`) — guards against the CN-mixup class of mistake flagged in [[02-Week 2 - TLS and mTLS/X.509 and PKI Concepts|X.509 and PKI Concepts]]' common-mistakes list.
...
`src/message.c`/`src/hmac.c` (`dd_serialize_message`/`dd_try_parse_message`/`dd_session_init`, plus a from-scratch `hmac_sha256` built on Week 1's own `sha256_context`) were written against `docs/PROTOCOL.md`'s exact spec. Reviewed line-by-line against the spec (byte offsets, HMAC coverage `[0, 12+N)`, key-derivation label, `seq_num` strictly-increasing rule, 65536-byte cap, failure-handling order) before ever compiling, then verified for real: built, linked, and run as a live client/server exchange over loopback, not just read for logical correctness.
...
### Finding 5: `server.c`'s echo can exceed `DD_MAX_BODY_LEN` — fixed (truncate) and verified

Decision (user's call, per this project's build-your-own-protocol-logic discipline): truncate the echoed body so `"ack: " + body` never exceeds `DD_MAX_BODY_LEN`, rather than dropping the prefix or failing the reply outright — a message this close to the cap is already an edge case on the sender's side, so losing a few trailing bytes of the *echo* is an acceptable tradeoff against dropping the whole reply (and connection) over it.

**Fix:** `reply_len` explicitly capped to `DD_MAX_BODY_LEN` in `handle_connection()`'s `TEXT_MESSAGE` branch, after the existing buffer-size check.

**Verified with a real boundary case, not just a code read:** sent a 65533-byte message (3 bytes under the cap) — small enough to be legal, large enough that `"ack: "` (5 bytes) pushes the natural echo to 65538, two bytes over. Server log confirmed the full message received intact (`Received TEXT_MESSAGE (seq=0, 65533 bytes)`, full 65533-byte body logged correctly), and the client's received reply measured out to exactly `DD_MAX_BODY_LEN` (65536) bytes — `"ack: "` plus 65531 of the original 65533 `A`s, the expected two-byte truncation, no more and no less. No crash, no dropped connection; the session's subsequent `DISCONNECT` was received and processed cleanly afterward, same as every other test.
...
### Not yet done (Day 3)
- A second device authenticated and tagged `tag:deaddrop-server`; confirming the ACL actually restricts traffic as intended (not just that this one device's tag looks right) needs that second device to exist.
...
### Fuzzing `dd_try_parse_message()` — the one genuinely new tool this week
...
- Pure-random buffers now span the **full valid message-size range** (up to `DD_MAX_MSG_SIZE`, ~64KB) instead of being capped at 512 bytes — the original range barely touched realistic message sizes at all
- Mutation fuzzing now runs against **three base messages of very different sizes** (44-byte, 4KB, and `DD_MAX_BODY_LEN - 1` bodies), not just one ~44-byte message — a bug reachable only on large-message code paths (buffer-boundary arithmetic near the max) would never have been reachable by mutating only a small message
...
1. **`global-buffer-overflow`** (from the original, smaller pass): a hand-counted string literal length (`44`) that didn't match the actual string (`42` characters). `dd_serialize_message()` correctly copied exactly the length it was told to; the harness lied about that length. **Fixed** with `strlen()`.
...
- **Decoupling**: the lock-screen state machine touches only ncurses windows and `lock.c` - never a socket, a `WOLFSSL*`, or `dd_session_state*` - so there's no code path by which it could disrupt the network/crypto layer.
...
**A real test-methodology bug found and fixed along the way**: the seeded PIN wasn't found on the first attempt after seeding - not a bug in `lock.c`, but in the test setup: the seeding tool ran as `connor` (writing to `/home/connor/.deaddrop/pin_hash`), but the server was launched via `sudo openvt`, and `sudo` resets `HOME` to `/root` by default - so the server (running as root) correctly found no PIN at `/root/.deaddrop/pin_hash` and started unlocked. Fixed by using `sudo --preserve-env=HOME env TERM=linux openvt ...` (preserving `HOME` specifically while still forcing the correct `TERM`) rather than assuming `sudo`'s environment handling would just work. Worth remembering for any future testing that mixes `sudo` with per-user state files.
...
- `$HOME/pki` - cert/key, **and** `revoked_serials.txt` (deliberately relocated here from `~/deaddrop/`, so a cert rotation's revocation-list update persists too)
...
- **Persisted message history** - confirmed two ways: (1) `msglog.txt`'s content matched exactly what was sent/received during testing; (2) a real service restart replayed it correctly on boot:
  ...
  [2026-08-22 19:17:58] client: (sent a file - see ~/.deaddrop/received/)
  --- end of previous history ---
  ```
- **File transfer** - confirmed end to end, including a real bug found and fixed along the way (see below): `bravo` sent `/tmp/testfile.txt` via `/send /tmp/testfile.txt`, `alpha` saved it to `~/.deaddrop/received/testfile.txt` with byte-for-byte correct content (`test attachment content`, 24 bytes, matching exactly).
...
## Touch keyboard attempt, real scrollback rewrite, and a pre-user-testing feature batch — 2026-08-22

### Touch keyboard: built, then deliberately abandoned
Given the previous section's touch-gesture re-verification, the user asked about adding a full on-screen touch keyboard. It was designed and built (a `KB_LAYOUT` grid of tappable keys feeding a "virtual keystroke" into `ui_poll_line()`'s existing per-mode key handling, so every mode's real logic - PIN checks, WiFi selection - stayed byte-for-byte reused rather than duplicated) and compiled clean, but real hardware testing found it unusable: keys didn't register taps, and the small key targets made it impractical regardless. Debugging that surfaced a much bigger, more important finding (see below), and the user ultimately decided to drop the touch keyboard entirely - physical mini keyboards are arriving soon, so it stopped being worth pursuing. The code was never committed and was fully reverted (`git checkout`) rather than left half-working in the tree.

### Real finding: touch hardware is not reliably reaching the kernel
Debugging the keyboard's dead taps eventually used `evtest` directly against `/dev/input/event1` (the confirmed ft5x06 device), bypassing the app entirely. Over multiple properly-instrumented capture windows (up to 90s, with the user tapping in real time), **zero raw touch events reached the kernel**, despite the user confirming they were physically tapping the correct device. Two of my own debugging mistakes complicated this investigation before landing on solid evidence, both worth recording:
- `fprintf(stderr, ...)` debug lines were, without realizing it at first, writing directly onto the physical console - the systemd unit sets `StandardError=tty` (`TTYPath=/dev/tty1`), the SAME tty ncurses owns, so every debug print was corrupting the live UI it was meant to help diagnose. Fixed by redirecting debug output to a plain file (`/tmp/ui_debug.log`) instead - obvious in hindsight, but cost real time before being caught.
- The first several `evtest` captures showed nothing because stdout was redirected to a file/pipe rather than a real terminal, and glibc defaults to fully-buffered (not line-buffered) I/O in that case - `timeout`'s SIGTERM killed the process before its buffer ever flushed. Fixed with `stdbuf -oL` for unbuffered output.

With a methodologically sound capture and zero events over a real-time-coordinated window, this is solid evidence of an actual hardware/driver-level fault, not an application bug. Per the user's explicit decision: **touch is being deliberately deprioritized/ignored for this testing phase, to be revisited in a later iteration** - not fixed, not silently dropped, documented here as a known, accepted gap.

### Real scrollback rewrite: history_win became an actual ncurses pad
Originally built to be driven by touch (tap upper/lower half to scroll), then re-pointed at the keyboard once the touch-reliability finding above made that the honest choice. `history_win` changed from a fixed-size `derwin()` (only ever showing what fit on screen, silently discarding anything that scrolled off - the "deliberately not true scrollback" limitation documented in the touch block comment before today) to a real `newpad()` retaining up to `HISTORY_PAD_LINES` (3000) lines. `history_total_lines`/`history_view_line`/`history_following` track position; `refresh_history_viewport_locked()` is now the single place that converts that state into an actual `prefresh()` call, replacing every old direct `wrefresh(history_win)` in the file. **UP/DOWN arrow keys** (only from `UI_MODE_NORMAL`) step the viewport half a screen at a time via `scroll_history_locked()`, snapping back to live auto-follow once caught up - the old touch-tap gesture in `handle_tap_locked()` for `UI_MODE_NORMAL` was removed outright (touch there is now a documented no-op), along with the earlier single-step pause/resume gesture it had itself replaced. The lock screen also got a real, incidental fix out of this: `draw_locked_overlay_locked()` now draws into a separate `lock_overlay_win` (same screen rectangle, different window) instead of `werase()`-ing `history_win` directly - this is what actually resolves the previously-deferred "locking wipes live scrollback" issue noted in the section above, since locking no longer touches the pad's buffer at all.

**Not yet live-tested** - built and compiles/links clean on real hardware (`-Wall -Wextra`, zero warnings, both Pis), but per the user's direction this and everything below is being left for the user-testing phase itself to exercise, not verified live in this session.

### Pre-user-testing feature batch (built, not yet live-tested - see above)
- **Message color differentiation**: `CP_OWN_MSG`/`CP_PEER_MSG` changed from cyan/white (too close to each other and to other UI elements after the earlier "soften the palette" pass) to green/yellow - a direct request so who-said-what is readable from color alone.
- **"alpha"/"bravo" terminology**: `client.c`/`server.c` previously hardcoded the literal peer/role labels `"client"`/`"server"` in `ui_init()`, `run_symmetric_session()`, and OLED text (`"DeadDrop client"`/`"DeadDrop server"`) - none of these were wired to the existing `-N <peer_hostname>` flag, they were separate hardcoded strings. Replaced with `"alpha"`/`"bravo"` throughout (`bravo` is the client role/connects-to-alpha; `alpha` is the server role/connects-to-bravo), matching this project's actual two-device naming everywhere else.
- **`/clear` and `/save <text>`** (`session.c`, `msglog.h`/`.c`, `ui.h`/`.c`): `msglog_append_saved()` tags a log line with a `[SAVED] ` marker; `msglog_clear_except_saved()` rewrites the log file in place (read + filtered temp-file write + `rename()`), keeping only saved-tagged lines - a real, on-disk zeroing, not just a screen clear. `ui_clear_history()` is the on-screen half: wipes the pad outright and replays whatever's left in the (now-filtered) log via a new shared `replay_msglog_into_pad_locked()` helper (factored out of `ui_init()`, which uses it too). `/clear` works both inside an active session (`session.c`) and while idle/disconnected (`idle_input_thread_main()` in `ui.c`) - a security-motivated "wipe my chat" command shouldn't require a live connection. `/save` is scoped to messages sent going forward only (no retroactive tagging of an already-logged line) - a documented v1 limitation, not an oversight.
- **LED flash-on-message now clears on keyboard activity too, not just touch**: given the touch-reliability finding above, relying on touch alone to acknowledge a pending-message LED flash could leave it flashing indefinitely on an affected device. New shared `clear_message_pending_flash()` helper, called from both `touch_thread_main()` (a real tap) and `ui_poll_line()` (any real keystroke) - either input method now works independently of the other's hardware health.
- **Quick help guide on every boot**: `ui_init()` now prints a short `--- Quick help ---` block (Ctrl+L, Ctrl+W, arrow keys, `/send`, `/save`, `/clear`, `quit`/`exit`) every process start (i.e. every boot, given `Restart=always`) - one `ui_add_history()` call per line, deliberately not a single multi-line call, since the real-scrollback math above depends on exactly one `history_total_lines` increment per call. A matching plain-text version was added to the non-Linux console fallback's `ui_init()` too, for consistency (though lock-screen/WiFi-setup lines are correctly omitted there - neither exists on that fallback).

### Regression suite + provisioning re-check, ahead of the user-testing phase
Per explicit instruction ("clean slate build for start of testing"), with touch explicitly out of scope for this pass and all of today's new UI features deferred to the user-testing phase itself:
- Full 5-binary suite (`test_aes128`, `test_hmac`, `test_sha256`, `test_revocation`, `test_message`) rebuilt from source (`-Wall -Wextra`, zero warnings) and re-run natively on **both** `alpha` and `bravo` (previously only re-run on `alpha`) - **5/5 pass on both**, zero regressions from today's changes.
- `docs/provision-permissions.sh` re-run on both devices - `chown`/`chmod` applied cleanly to `~/pki`, `~/keyshare`, `~/.deaddrop` on both, no ownership drift found this time (unlike the real bug this script was written to catch earlier in the project).
- Both `deaddrop-alpha.service`/`deaddrop-bravo.service` confirmed `active (running)` on the latest build after every rebuild in this section.

### Real bug found and fixed: scrollback line-count didn't account for wrapped lines
Before starting user testing, both Pis were reset for a genuine out-of-box first-boot experience: `~/.deaddrop/message_log.txt`, `~/.deaddrop/pin_hash`, and `~/.deaddrop/received/*` deleted on both (leaving `~/pki`/`~/keyshare`/network config untouched - those are infrastructure, not chat state), services restarted. Dumping the real physical console (`/dev/vcs1`) on both to confirm - `alpha` looked correct (fresh quick-help guide, "No PIN set" prompt, no stale history), but **`bravo`'s screen had already scrolled the quick-help guide off-screen**, despite having nowhere near a viewport's worth of genuinely separate lines to justify it.

Root cause: every site that increments `history_total_lines` (see the real-scrollback rewrite above) used a flat `+1` per `ui_add_history()`/replay/WiFi-list call, on the assumption that one call = one physical pad row. That's wrong once a line's total display width exceeds the pad's column width - ncurses wraps a `wprintw()` write at the window edge, consuming MULTIPLE physical rows for what's still only one logical `history_total_lines` increment. This project's own session-start hint text is ~240 characters, more than double a 98-column window's width - undercounting by that much per occurrence was enough to throw `history_view_line`'s live-position math off after only a handful of lines, exactly matching what was observed on `bravo`.

Fixed with a shared `wrapped_row_count(display_width, window_width)` helper (`ceil(display_width / window_width)`, minimum 1), used at every increment site instead of a flat `+1` - `ui_add_history()` (computing the full timestamp+prefix+text width), `replay_msglog_into_pad_locked()` (per replayed log line), and the WiFi-list renderer (header + each entry). Rebuilt (`-Wall -Wextra`, zero warnings on real hardware), both Pis re-wiped and restarted again, both consoles re-dumped directly - `bravo` now shows the full quick-help guide identically to `alpha`.

## Real bug: silent peer loss never detected (LED never turns red) - root-caused and fixed — 2026-08-22

The user reported that unplugging one Pi didn't turn the other's status LED red. Investigating this properly surfaced a genuine, previously-undiscovered bug in the connection-liveness design, plus a full LED color-scheme redesign, plus a batch of UI-noise/color-differentiation requests. All covered here together since they were worked in the same session and touch the same files.

### LED color-scheme redesign
`hw_expansion.h`'s `HW_STATUS_ALERT` (a single magenta "message pending" flash color, alternated against LED-off) replaced with `HW_STATUS_MSG_CONNECTED` (blue) and `HW_STATUS_MSG_DISCONNECTED` (orange) - `ui.c`'s flash loop now alternates between the appropriate alert color and the CURRENT base connection color (green/red), never LED-off, so link-up/down state stays visible throughout the flash, not just readable during the "off" phase. A new `ui_set_link_state(int connected)` (`ui.h`/`ui.c`) keeps a `hw_link_connected` flag in `ui.c` in sync, called from `client.c`/`server.c` at every existing `HW_STATUS_CONNECTED`/`HW_STATUS_DISCONNECTED` call site (4 in `client.c`, 2 in `server.c`). Final scheme: solid green (connected, no message), flashing blue/green (connected, unread message), flashing orange/red (disconnected, unread message), solid red (disconnected). `clear_message_pending_flash()` (already built for keyboard-based acknowledgment) now restores the CORRECT base color depending on `hw_link_connected`, not a hardcoded green.

### Real bug found and fixed: a silent peer loss was never detected - not slowly, literally never
Investigated with real, controlled live tests rather than guessing:
- **Graceful close** (`systemctl stop` on the peer, which sends a real TCP FIN): detected **instantly** - confirmed via console timestamps matching to the same second (`23:01:39` stop, `23:01:39` "Connection to bravo lost."). This path was never broken.
- **Silent loss** (an `iptables DROP` rule on the peer, silently discarding all traffic to/from the port, simulating a real power-loss unplug where no FIN/RST is ever sent): **never detected, not even after 2+ minutes**, despite `CONN_TIMEOUT_SECONDS` (30s) being set via `SO_RCVTIMEO` on the socket.

Root-caused properly, not guessed at, across several real diagnostic steps:
1. `gdb -p <pid> -batch -ex "thread apply all bt"` on the live, stuck process confirmed the receiver thread genuinely blocked inside `wolfIO_Recv()` → the kernel's own `recvfrom()` syscall, 80+ seconds in.
2. An initial hypothesis (wolfSSL's `EmbedReceive()` retrying internally on `EWOULDBLOCK` from a blocking socket it doesn't know might time out - a real, documented wolfSSL quirk, [wolfSSL/wolfssl#1561](https://github.com/wolfSSL/wolfssl/issues/1561)) led to trying `wolfSSL_set_using_nonblock(ssl, 1)` after the handshake - **did not fix it**, confirmed via a clean, properly-isolated retest (this first retest was itself confounded by a leftover `iptables` rule from an earlier test round that hadn't been cleaned up - caught and corrected before trusting the result).
3. A direct `getsockopt(SO_RCVTIMEO)` check added at the exact point `set_socket_timeout()` runs confirmed the timeout genuinely WAS `30s` at handshake time (`tv_sec=30`) - ruling out "the value never got set" as the cause.
4. Reading `session.c`'s own `clear_recv_timeout()` function (and its already-existing comment, previously not connected to this bug) revealed the actual root cause: it deliberately resets `SO_RCVTIMEO` to 0 (block indefinitely) the moment a session goes live, specifically so a normal idle chat session doesn't spuriously disconnect every 30s of real silence - a correct design choice for "peer is fine, just quiet," which simply leaves no read timeout at all for "peer's network silently died." An older comment on `server.c`'s `CONN_TIMEOUT_SECONDS` claiming it "doubles as" a dead-peer detector was simply wrong and has been corrected in place.

**Fixed with an application-level watchdog** (`session.c`), not by fighting wolfSSL's blocking-I/O semantics further: `shared_session_ctx` gained `last_recv_epoch` (updated by the receiver thread on every successfully parsed message, of any type). The sender loop - which already wakes up regularly to send a periodic PING for RTT reporting - now also checks `time(NULL) - ctx.last_recv_epoch` every iteration; past `SESSION_WATCHDOG_TIMEOUT_SECONDS` (25s), it calls `SHUTDOWN_READ(sock)` - the exact same mechanism this file already uses to unblock a receiver thread stuck in a blocking read when the local user quits - which forces the blocked `recv()` to return immediately and unconditionally, regardless of wolfSSL's internal retry behavior. The (harmless but ultimately unhelpful) `wolfSSL_set_using_nonblock()` calls from step 2 were removed rather than left in as dead diagnostic-era code.

**Verified with a second clean live test**: silent `iptables DROP` re-enabled at `23:26:49`, `[23:27:08] Connection to bravo lost.` on `alpha`'s real console - detected in **19 seconds**, well inside the 25s watchdog window (the old behavior: still not detected after 2+ minutes, i.e. never). Status bar correctly reverted to `Listening on port 4433...`, confirming the full disconnect path (including the LED going red) completed. `iptables` rules removed, both devices reconnected cleanly afterward.

### RGB fan research (Freenove FNK0100)
Checked directly against Freenove's own published source (`api_expansion.py`, `task_fan.py` in `github.com/Freenove/Freenove_Computer_Case_Kit_for_Raspberry_Pi`), not assumed: the case's fans are controlled via `REG_FAN_MODE`/`REG_FAN_FREQUENCY`/`REG_FAN_DUTY`/`REG_FAN_THRESHOLD` - PWM speed and temperature-following curves only. **No color/RGB register exists for the fans on this board** - only the separate RGB LED registers (`REG_LED_SPECIFIED`/`REG_LED_ALL`/`REG_LED_MODE`, already implemented in `hw_expansion.c`) support color. Not pursued further - there's nothing to implement here, the hardware doesn't support it.

### UI noise reduction and expanded color-coding
- **Certificate-revocation verify-callback messages quieted**: `server.c`'s `my_verify_callback()` used to print "checking serial X against revocation list" and "serial X not revoked, proceeding" on EVERY connection. Not a security risk in itself (a cert serial number is public by design, sent in the clear as part of any TLS handshake - the message never exposed anything an attacker couldn't already read off the certificate or a packet capture), but pure noise, since revocation checking still happens every time regardless of whether it's displayed. The REJECTED-cert case (`is REVOKED - rejecting`) stays visible - that's a real security event worth seeing, not routine confirmation.
- **Per-connection instructional hint removed**: `session.c`'s `run_symmetric_session()` used to print the full "Type a message and press Enter..." paragraph on every single connect/reconnect - a flaky link doing several reconnect cycles spammed the same instructions repeatedly. Factored into a new shared `ui_show_help()` (`ui.c`/`ui.h`), called once at boot (`ui_init()`) and on demand via a new **`/help`** command (works both inside an active session and offline, same as `/clear`).
- **Expanded color-coding**: two new color pairs, `CP_TIMESTAMP` (blue - previously shared `CP_SYSTEM`, now visually distinct metadata) and `CP_ERROR` (red - new). New `ui_add_error()`/`ui_add_errorf()` (`ui.h`/`ui.c`, sibling to `ui_add_history()`/`ui_add_historyf()`) render in `CP_ERROR` instead of the routine system color. Converted every genuine failure call site across `client.c`, `server.c`, `session.c`, and `ui.c` (socket/bind/listen/accept/connect failures, handshake failures, verify-callback failures, send/receive failures, `/send` failures, out-of-memory, WiFi connect/scan failures, queue-full) - routine status/confirmation messages (`Connected to X`, `(sent file: ...)`, `(delivered: ...)`, `(you quit)`) deliberately left as the normal system color, only genuine failures got the new red treatment.
...
- `~/.deaddrop/message_log.txt` - does not exist on either device (confirmed via `ls`, not just "the app said so").
- `~/.deaddrop/` - completely empty on both (just `.`/`..`).
- `grep -r "TESTMSG" ~/.deaddrop/` - zero hits on both.
...
## Real bug: boot hangs forever on an unrecognized WiFi network — 2026-08-23

User report: "when going from a known wifi, to a unknown wifi upon boot, boot sequence doesnt quite finish." A real, reproducible field bug, not a misunderstanding.

### Root cause
`ui_init()` and `ui_start_idle_input()` - the calls that bring up the ncurses UI and start the background thread that services `Ctrl+W` (WiFi setup) and `Ctrl+L` (lock screen) - used to run only AFTER certificate/key/CA loading had fully succeeded in both `client.c` and `server.c`. That loading path includes `load_private_key()` -> `keyshare_reconstruct()` (`keyshare.c`), whose retry loop is a deliberate `for (;;)` with exponential backoff (`BACKOFF_INITIAL_SECONDS=1`, doubling, capped at `BACKOFF_MAX_SECONDS=30`) and **no overall timeout** - it's designed to keep retrying forever until it reaches the paired peer over Tailscale. If the device has no network path to its peer at boot at all - e.g. it's in range of only an unrecognized WiFi network that NetworkManager won't auto-join, exactly the user's report - the entire boot sequence stalls forever with no UI ever drawn, no WiFi-setup screen reachable, and no way for anyone to intervene short of a power cycle (which just repeats the same hang).

### Fix
Moved `ui_init(...)` + `ui_start_idle_input()` in both `client.c` and `server.c` to run immediately after `wolfSSL_CTX_new(...)` succeeds - before any certificate/key/CA loading, including the network-dependent `load_private_key()` call. This means the idle-input thread is already running and genuinely responsive on its own thread while the main thread blocks inside `load_private_key()`/`keyshare_reconstruct()`. A real "only an unrecognized WiFi network in range" scenario is now recoverable from the touchscreen/keyboard (enter PIN if locked, then `Ctrl+W` for WiFi setup) instead of hanging the boot indefinitely.

Since credential loading now runs after ncurses is active, every `printf`/`fprintf` between the old and new `ui_init()` position - including every print statement inside `load_private_key()` itself, in both files - was converted to `ui_add_history()`/`ui_add_error()`/`ui_add_errorf()`, matching this project's already-established "a stray direct print once ncurses is active corrupts the screen it manages" rule. The original "nothing printed here includes a filesystem path" security property is unaffected - none of these messages ever included a raw path value, before or after the move; only the output mechanism changed. `client.c` also needed an explicit `ui_set_status("Loading credentials...")` right after the move (since `ui_init()` itself sets a generic "Connecting to..." status that's now premature) and `ui_set_status("Connecting to alpha...")` restored right before the reconnect loop once credential loading actually finishes; `server.c` didn't need this - its existing post-`listen()` `ui_set_statusf("Listening on port %d...", SERVER_PORT)` already provides the correct transition message. `revocation_load()` in `server.c` (runs even earlier, before `ctx` exists, doesn't block on network) was left untouched.

### Verified by reproducing the actual failure, not just re-confirming normal boot
Rather than trusting "normal boot still works," the fix was validated against a real, deliberately-forced version of the reported scenario. On `bravo`: `sudo iptables -A OUTPUT -d 100.124.177.123 -j DROP` and the matching `INPUT` rule, blocking ALL traffic to/from `alpha`'s Tailscale IP - simulating a peer that's genuinely unreachable at boot, the same network condition an unrecognized-WiFi boot produces. Service restarted with the block active.

A first check (before a fully clean rebuild) showed the fix appearing NOT to work - `ui_active=0`, only 2 threads, plain-text console output - despite `grep` confirming the deployed source already had the fix. Diagnosed as a stale incremental build; fixed by `rm -f build/client.o build/client` and a full clean recompile+relink, confirmed via a fresh `.o` timestamp before retesting.

With the clean binary and the same iptables block still active: `ps -T -p <pid>` showed **4 threads** (main, keyshare-listener, idle-input, touch), and `sudo gdb -p <pid> -batch -ex "print ui_active" -ex "print idle_thread_running" -ex "print touch_thread_running" -ex "thread apply all bt"` confirmed `ui_active=1`, `idle_thread_running=1`, `touch_thread_running=1`, with the idle-input thread genuinely blocked in `wgetch` (i.e. actively waiting on real keyboard input, not dead) and the touch thread actively polling, while the main thread was correctly stuck exactly where expected: `connect()` -> `fetch_remote_share` -> `keyshare_reconstruct` -> `load_private_key` (`client.c:291`). `sudo cat /dev/vcs1` confirmed the physical screen was a fully-rendered, interactive LOCKED/PIN-entry prompt with the status bar correctly reading `Loading credentials...` - definitive, hardware-verified proof the UI stays up and interactive while the keyshare fetch blocks indefinitely against an unreachable peer, exactly the fix intended.

Test cleanup: `iptables` rules removed from `bravo`, both services restarted. `bravo` reconnected to `alpha` cleanly (`Connected to alpha` in the status bar) with no lingering block. `server.c`'s matching fix was then deployed to `alpha` the same way (clean compile/link, zero warnings on `gcc -Wall -Wextra -g`) and restarted; its console confirmed the new `ui_add_history()`-routed messages ("Fetching this device's key-share...", "Key-share reconstructed.", "wolfSSL initialized...") rendering correctly through the UI rather than corrupting the screen, then connecting to `bravo` normally - both devices left in a clean, fully-connected working state.

### Known related gap, not yet fixed
`keyshare.c`'s `fetch_remote_share()` still uses a plain blocking `connect()` with no timeout of its own. The fix above makes the UI usable while `keyshare_reconstruct()`'s retry loop blocks, but a single `connect()` attempt inside that loop could still hang for a long time against a host that's routable but unresponsive (as opposed to genuinely unreachable, which fails fast) - structurally the same issue `client.c`'s TLS-connect already had fixed earlier this project via `connect_with_timeout()`. Not yet requested by the user; flagged here for a future pass.

### Separately noticed, deliberately parked
During this investigation's connectivity troubleshooting, `bravo`'s console geometry (`stty -F /dev/tty1 size` -> 20 rows x 66 cols) was found to not match `alpha`'s long-established 30x100 baseline - user independently observed "bravo has a larger font size than alpha." Traced to `fbset -fb /dev/fb0` showing a genuinely different physical panel resolution (`bravo`: 800x480 vs `alpha`: 1280x720, and 800/12 x 480/24 exactly matches bravo's wrong dimensions using the correctly-configured 12x24 Terminus font) - not a font-config or DSI-binding regression (`/etc/default/console-setup` and `/boot/firmware/config.txt` confirmed byte-identical on both devices, `dmesg` showed no explicit panel/EDID errors on either side). Most likely either a genuine physical panel difference between the two units or DSI auto-detection flakiness from a specific boot. Not fixed - parked for a future pass, unrelated to the WiFi-boot-hang bug this entry is about.

## Automatic WiFi setup on boot when there's no connectivity — 2026-08-23

Direct follow-up request after the boot-hang fix above: the user didn't just want the WiFi-setup screen to be *reachable* via Ctrl+W while credential loading blocks - they wanted the device to actually detect "no connectivity" and jump into WiFi setup **automatically**, with no keypress required, and without the retry activity happening elsewhere (the keyshare fetch, status-bar updates, etc.) interrupting or fighting that flow.

### Design
`ui.c` already had two of the three pieces needed: `wifi_has_connectivity()` (`wifi.c`, wraps `nmcli networking connectivity`) and the WiFi scan/select/connect flow itself, previously reachable only via the Ctrl+W keystroke handler inside `ui_poll_line()`. The scan-and-render logic (run `wifi_scan()`, draw the results, enter `UI_MODE_WIFI_SELECT`, or fall back to `UI_MODE_NORMAL` with an error if nothing was found) was factored out of that keystroke handler into a new standalone function, `perform_wifi_scan_flow()`, so it could be called from a second, non-keystroke entry point without duplicating ~55 lines of rendering logic. The Ctrl+W path now just calls it.

The new entry point sits at the very top of `idle_input_thread_main()`, before its main loop starts: under `ui_mutex`, if `ui_mode == UI_MODE_NORMAL` (never true if a PIN-locked screen is showing - same guard Ctrl+W's manual entry already uses, so a locked device still can't be walked into WiFi setup without unlocking first) and `wifi_has_connectivity()` is false, it sets `ui_mode = UI_MODE_WIFI_SCANNING`, posts a history line explaining what's happening, and calls `perform_wifi_scan_flow()` - the exact same code path Ctrl+W uses, just triggered automatically instead of waiting on a keypress.

**Avoiding the "interruption/cycling" the user explicitly asked to avoid**: this only runs ONCE, at idle-thread startup - not folded into the existing periodic `UI_CONNECTIVITY_CHECK_SECONDS` (15s) re-check further down in the same loop. If it were, cancelling out of WiFi setup (Ctrl+W) while still genuinely unconnected would get the user shoved right back into it 15 seconds later, against their will - exactly the unwanted cycling this was built to prevent, not cause. A cancelled-out user still gets the existing passive status-bar hint from the periodic check ("No network found - press Ctrl+W..."), they just aren't forced back into the flow.

**Why nothing else can actually interrupt this flow**: the main thread's `load_private_key()`/`keyshare_reconstruct()` retry loop (the thing that used to block the whole UI before the earlier fix) runs on a completely separate thread and never touches `ui_mode`, `history_win`, or any of the redraw calls this flow uses - it can set the status-bar *text* (`ui_set_status()`) concurrently, which is a known, already-accepted cosmetic-only race (documented where `UI_CONNECTIVITY_CHECK_SECONDS` is defined), but it cannot change *what mode the screen is in* or knock the user out of WiFi setup. The touch thread is the only other thing running this early (started inside `ui_init()` itself, before `ui_start_idle_input()` returns) and is guarded by the same `ui_mutex`.

### Verified live, without disconnecting the dev machine's own SSH access
Simulating "no connectivity" safely was the tricky part - actually turning off WiFi on either Pi would have killed SSH access to it too (no ethernet connected on either device). Blocking NetworkManager's own connectivity-check target IPs via `iptables` was tried first but didn't reliably flip `nmcli networking connectivity` away from `full` even after a forced recheck (likely a CDN-backed check URL resolving to different IPs than the ones blocked) - abandoned in favor of a cleaner method: a scratch copy of `ui.c` with a test-only env-var override (`DD_FORCE_NO_WIFI=1` forces the boot-time check to behave as if `wifi_has_connectivity()` returned false, real scan/render logic untouched), built into a throwaway test binary, run manually attached to `alpha`'s real `/dev/tty1` (service stopped first) with real production arguments.

Console output confirmed the complete intended sequence, in order: `"No network detected at boot - starting WiFi setup automatically (Ctrl+W to cancel)."`, then `"Scanning for WiFi networks... (Ctrl+W to cancel)"`, then a real scan result - `WiFi networks found (Ctrl+W to cancel): 1) The Arrow 802.1X (secured), 2) The Arrow (secured), 3) BDH-public (secured)` - correctly landing in `UI_MODE_WIFI_SELECT` with `"Select network number (Ctrl+W to cancel):"` as the input prompt. Critically, **all of this happened while the keyshare fetch and mTLS handshake completed normally in the background on the main thread** - the same console capture shows `"Key-share reconstructed."`, `"wolfSSL initialized; certificate, key, and CA loaded."`, and the status bar ending on `"Connected to bravo"` - direct, real proof the automatic WiFi-setup flow and the peer-connect retry loop ran fully concurrently with zero interference in either direction, exactly as designed.

Test binary killed and scratch files deleted afterward; the real (non-test-hook) `ui.c` was then compiled clean (`gcc -Wall -Wextra -g`, zero warnings) into fresh `build/server`/`build/client` on both `alpha` and `bravo`, and both services restarted. With real connectivity present on both (still on `BDH-public` at this point), the automatic trigger correctly stayed silent on both - straight to `Key-share reconstructed.` / `wolfSSL initialized...` / `Connected to <peer>` with no WiFi-setup detour, `NRestarts=0` on both, confirming the feature only activates when it's actually supposed to.

## Real bug: Ctrl+W's own "connect to a new network" silently failed on every real attempt — 2026-08-23

Direct follow-up while helping the user actually move both devices onto their apartment's real managed WiFi (WhiteSky-provisioned, SSID `The Arrow` - MAC-registered via a resident portal, open at the 802.11 layer with access enforced by the backend, distinct from the separate `The Arrow 802.1X` enterprise-auth SSID at the same APs). A raw `nmcli device wifi connect "The Arrow"` run manually (unprivileged) over SSH failed with `Error: Failed to add/activate new connection: Not authorized to control networking.` - re-run with `sudo` it succeeded cleanly in under 10 seconds (full association + DHCP). That's the same command `wifi_connect()` (`wifi.c`) issues from Ctrl+W's connect flow, and the real systemd service runs as an unprivileged user (`User=connor`), never `root`.

### Root cause, confirmed directly from `journalctl -u NetworkManager`, not assumed
```
audit: op="connection-add-activate" pid=1928 uid=1000 result="fail" reason="Not authorized to control networking."
audit: op="connection-add-activate" pid=2098 uid=1000 result="fail" reason="Not authorized to control networking."
audit: op="connection-add-activate" pid=2216 uid=1000 result="fail" reason="Not authorized to control networking."
...
audit: op="connection-add-activate" uuid="164a2b9f-..." name="The Arrow" pid=2264 uid=0 result="success"
```
Three real, distinct failed attempts (uid=1000, `connor`) - almost certainly the user's own Ctrl+W attempts from the actual running app - immediately followed by one success the moment the identical operation ran as `uid=0`. NetworkManager's polkit policy treats *activating an already-known connection profile* (what happens automatically for `netplan-wlan0-BDH-public` at boot - no user interaction, no privilege needed) very differently from *adding a brand-new profile for an SSID nmcli has never seen before* (`connection-add-activate` - what every single Ctrl+W connection to a genuinely new network requires), and only the first is permitted for a regular console user by default. This means the "connect to a new network" half of the WiFi setup feature has likely never actually worked end-to-end in the real deployed service, on any SSID - not a bug specific to `The Arrow`, not a timeout, not a slow-DHCP issue (the underlying connection itself completed in seconds once properly authorized).

### Fix
`wifi_connect()` now prefixes its `nmcli device wifi connect` call with `sudo` (relies on this project's own already-established NOPASSWD sudoers entry - the same one every other elevated command this project runs depends on, never an interactive password prompt). This required also fixing `run_nmcli()` itself, which had `execvp("nmcli", argv)` hardcoded regardless of `argv[0]` - previously any `sudo` prefix placed in `argv[0]` would have been silently ignored (still execing plain `nmcli` unprivileged, defeating the fix entirely without any visible error). Changed to `execvp(argv[0], argv)` so `argv[0]` genuinely controls what actually runs. `wifi_scan()`/`wifi_has_connectivity()` were left unprivileged - confirmed working fine as `connor` throughout this entire session's testing, so only the one operation that actually needs elevation got it.

### Verified two ways
1. **Functional, on real hardware, through actual client isolation**: `alpha`'s saved `The Arrow` profile was deleted (`nmcli connection delete`, forcing a genuinely fresh `connection-add-activate` on the next attempt, not a cached/already-known reactivation) and a throwaway standalone test harness (`wifi_connect_test.c`, calls the real fixed `wifi_connect()` directly, no ncurses/synthetic keyboard needed) was run as the unprivileged `connor` user, targeting `The Arrow`. The dev machine lost its own SSH session mid-test (expected - the call itself switches `wlan0` away from whatever network carried that SSH connection) and, once `The Arrow`'s confirmed client/AP isolation made both mDNS and even Tailscale-routed SSH unreachable from the dev machine afterward, the *only* remaining way to confirm the outcome was the user's own eyes on the device: `alpha`'s status bar showed it successfully on `The Arrow` and `Connected to bravo` - a transition that could only have been caused by the fixed `wifi_connect()` call succeeding as the unprivileged user, since the saved profile had just been deleted and nothing else on the box would have reconnected it.
2. **Deployed for real** (not just the standalone test binary): `wifi.c` recompiled clean (`gcc -Wall -Wextra -g`, zero warnings) into fresh `build/server`/`build/client` and redeployed on `bravo` while it remained reachable. `alpha`'s real binary deployment was blocked mid-session by the same client-isolation/Tailscale-ACL access problem described above (see "Separately noticed" below) - left for the user to complete directly via keyboard access to the device, since the fix was already on GitHub (`git pull` away) by that point.

### Separately noticed: `The Arrow`'s client isolation plus a Tailscale ACL gap blocked remote debugging entirely
Mid-verification, the dev machine and `alpha` ended up on the same SSID (`The Arrow`) but still couldn't reach each other at all - confirmed genuinely isolated (not a transient failure): `tailscale ping` succeeded cleanly (`pong ... in 47ms`, WireGuard tunnel itself fully healthy) while SSH (TCP:22) consistently timed out, both over the direct LAN path (blocked by the property's own AP/client isolation, exactly as the user had flagged in advance from prior experience with this provider) and over the Tailscale-routed path (the tunnel works, but something - almost certainly this project's own already-existing, deliberately narrow `docs/tailscale-acl.json`, scoped to the app's own ports rather than SSH - doesn't permit port 22 between these tailnet peers). The dev machine had to fall back to `BDH-public` (unisolated) to regain any remote access at all, which only worked for `bravo` (still on `BDH-public`) - `alpha` (now on the isolated `The Arrow`) stayed unreachable from the dev machine for the rest of this session as a direct result. Not fixed here - either loosening the ACL to permit SSH, or accepting that debugging a device on an isolated network always requires physical/local access, is a real decision for later, not an oversight.

## Two more real bugs found the same night: association/scan-cache retry, and unreliable scan coverage — 2026-08-23

Continued live hot-swap testing (both devices being moved between `BDH-public` and `The Arrow` repeatedly by hand) surfaced two more real, distinct bugs once the permission fix above stopped masking everything else.

### Bug 1: a quick Ctrl+W retry after a failed connect can fail differently, not just repeat the same error
Confirmed via `journalctl -u NetworkManager` on `bravo`: a first Ctrl+W attempt to `The Arrow` succeeded on the permission front (`uid=0`, the fix above genuinely working) but failed at the 802.11 layer - `Activation: (wifi) association took too long, failing activation` (`reason='supplicant-timeout'`), a real RF/AP-side timing issue, not a code bug. The natural next thing a user does - press Ctrl+W again right away - then failed with a *different* error: `reason="Failed to determine AP security information"`. NetworkManager throttles how often `nmcli device wifi list` actually triggers a fresh radio scan; a retry soon enough after a failed association can land on a stale or momentarily-invalidated cache entry for that specific SSID, and nmcli refuses to guess at what security type to build the new connection profile with.

**Fix**: `wifi_connect()` now issues a best-effort `nmcli device wifi rescan` immediately before the actual connect attempt, with a short settle delay, so the immediately-prior scan is genuinely fresh right when it matters - rather than trusting whatever was cached from the original Ctrl+W scan (and however long the user took entering a password) ago.

### Bug 2: a single scan pass doesn't reliably find every network actually in range
Direct user report while hot-swapping networks by hand: "need networkmanager to scan multiple seconds for ssids, not just a single flash scan. its not always picking up all networks." Root cause matches Bug 1's underlying mechanism: `wifi_scan()` (the function behind both the automatic no-connectivity-at-boot trigger and every manual Ctrl+W scan) went straight to `nmcli device wifi list` with no rescan of its own at all - just reading whatever NetworkManager already had cached. A real 802.11 scan has to dwell on every channel it probes (this project's own earlier scan results showed 11+ distinct frequencies across both bands from a single property), which genuinely takes several real seconds to cover - a "read whatever's cached right now" call can easily miss a network that's genuinely in range but wasn't caught by whatever scan happened to run last.

**Fix**: `wifi_scan()` now forces a real rescan (`nmcli device wifi rescan`) and waits `WIFI_SCAN_SETTLE_SECONDS` (4s) for it to actually finish covering both bands before reading results - same best-effort pattern as Bug 1's fix (if the rescan request itself fails, e.g. throttled by an immediately-repeated call, falls straight through to reading whatever's cached anyway rather than aborting the scan entirely).

### Deployed and verified running on both devices
Both fixes compiled clean (`gcc -Wall -Wextra -g`, zero warnings) and deployed for real (fresh `build/server`/`build/client`, not just a standalone test) to both `alpha` and `bravo` across several real network hot-swaps between `BDH-public` and `The Arrow` during this session - confirmed via `/dev/vcs1` console dumps and `systemctl show ... -p NRestarts` (`0` on both) that each device came back up cleanly and reconnected to its peer (`Connected to alpha`/`Connected to bravo`) after every redeploy, with no regressions from the added scan/retry delays.

### Reinforces the isolation/ACL finding above
Getting these fixes onto both devices required repeatedly moving the dev machine between `BDH-public` and `The Arrow` to chase whichever device needed patching next - directly motivated by, and now the clearest illustration yet of, the access gap noted above: there is currently no reliable way to reach a device on `The Arrow` for remote debugging without also being on `The Arrow` at that exact moment, and even then it isn't guaranteed (isolation behavior appeared inconsistent - reachable at some points, not others, possibly per-AP rather than network-wide on this property's multi-AP deployment). Loosening the Tailscale ACL to permit SSH between these tagged devices and the dev machine would remove the need for this network hot-swapping entirely and is worth real consideration before the next round of field testing.

## Third instance of the same permission gap: `nmcli device wifi rescan` also needs `sudo` — 2026-08-23

Direct user report immediately after the two fixes above went live: on `bravo`, Ctrl+W's scan only ever showed the currently-connected SSID (`BDH-public`) - until manually reconnecting to that same network via Ctrl+W's own connect flow, after which a subsequent Ctrl+W scan suddenly showed everything correctly (`The Arrow`, `The Arrow 802.1X`, etc). That "reconnect fixes it" pattern was the key clue.

### Root cause
Both rescan fixes added earlier tonight (`wifi_scan()`'s own rescan, and `wifi_connect()`'s pre-connect rescan) called plain `nmcli device wifi rescan` with no `sudo` - the exact same unprivileged-user gap `wifi_connect()`'s main connect call had, just not yet found for this specific nmcli subcommand. Confirmed directly, unprivileged, on real hardware: `Error: org.freedesktop.NetworkManager.wifi.scan request failed: not authorized.` Both rescan call sites are deliberately best-effort (silently fall through to reading cached results on failure, since NetworkManager also throttles legitimate rapid rescans and that's supposed to be tolerated gracefully) - which is exactly why this was invisible: the call didn't crash or show an error anywhere, it just silently never worked, at all, for this unprivileged user, indistinguishable from "NetworkManager decided to throttle it this time" without checking the actual return value by hand. The "reconnect fixes it" behavior makes sense in hindsight: a successful `sudo`'d `wifi_connect()` call (main connect, not the rescan) refreshes NetworkManager's own internal scan state as a side effect of actually activating a connection, incidentally working around the rescan's own silent failure.

### Fix
Both `rescan_argv` arrays (`wifi_scan()` and `wifi_connect()`) now prefix `sudo`, identical to the main connect call's own fix earlier tonight.

### Verified live, real Ctrl+W flow, not just manual nmcli
`sudo nmcli device wifi rescan` run by hand on `bravo` confirmed exit 0 with zero error output (versus the unprivileged `not authorized` error moments earlier) before touching the app at all. Deployed for real (fresh `build/server`/`build/client`, restarted, `NRestarts=0`), then verified through the actual UI - not a standalone test harness this time: a synthetic-keyboard tool sent a single real Ctrl+W chord, and the console showed all three real networks (`BDH-public`, `The Arrow 802.1X`, `The Arrow`) on the very first scan, with no reconnect-first workaround needed. Second Ctrl+W cancelled cleanly back to the normal compose prompt; test tooling deleted afterward.

## Fourth real bug: open networks misreported as "secured" — 2026-08-23

Found while preparing a full, genuine Ctrl+W hotswap test (user asked for direct verification "using the pi's functions" that a real network swap completes with zero errors, not just the scan). `wifi_scan()`'s security-detection logic was written against `nmcli`'s *human-readable* SECURITY column, which prints `--` for an open network - but `wifi_scan()` actually uses terse (`-t`) output, which uses a different convention entirely: an open network's SECURITY field is simply **empty**, confirmed byte-for-byte on real hardware (`nmcli -t -f SSID,SECURITY device wifi list | cat -A` showed `The Arrow:$` - nothing between the colon and end-of-line, not `The Arrow:--$`). The old check, `strstr(p, "--") == NULL`, evaluates true for an empty string too (there's no `"--"` to find in `""` either), so every genuinely open network was silently misreported as `secured` - the UI would prompt for a password it should never have asked for, which `wifi_connect()` would then pass to an open network's connect call.

**Fix**: `secured = (p[0] != '\0')` - a genuinely non-empty security string means secured (WPA1/2/3, WEP, 802.1X, ...), an empty one means open, matching terse mode's real convention rather than the human-readable one this was originally written against.

### Verified as part of a full, real, bidirectional hotswap test
With all four fixes deployed, ran the complete flow the user explicitly asked to see verified - the Pi's own Ctrl+W function actually switching networks, not a manual nmcli call - via a synthetic-keyboard tool driving a real Ctrl+W press, a real several-second scan, and a typed network selection + Enter, watched end-to-end with a live `journalctl -u NetworkManager -f` capture running throughout.

`alpha`, starting on `The Arrow`: Ctrl+W correctly showed `1) The Arrow (open)`, `2) The Arrow 802.1X (secured)`, `3) BDH-public (open)` - the security-detection fix confirmed correct on real, current scan data, not just reasoned about. Selected `BDH-public` (3): NetworkManager log showed a completely clean transition (`connection-activate ... uid=0 result="success"` through `Activation: successful, device activated.`, zero errors, zero "not authorized", zero warnings) and the console confirmed `Connected to WiFi network "BDH-public".` - the dev machine independently confirmed the device was actually on the new network from the outside. Then the reverse: selected `The Arrow` (1) from a fresh scan while on `BDH-public` - identical clean result, `Connected to WiFi network "The Arrow".`, confirmed from the dev machine again. **Both directions, zero errors, and `Connected to bravo` never dropped from the status bar through either swap** - the app-level peer connection survived both network transitions cleanly. Test tooling and log captures deleted afterward.

This is the first time this session the actual Ctrl+W-driven hotswap - the literal feature originally reported as broken - was confirmed working end-to-end, bidirectionally, from a source independent of the device itself (the dev machine watching from outside), rather than inferred from logs or a standalone test harness.

## Real bug: boot sequence overlaps the splash banner on a hard power cycle — 2026-08-23

Direct user report from an actual unplug/replug power cycle (not a `sudo reboot`, a real cold boot): "the boot sequence overlaps onto the banner." This is the same underlying cause this project already found and documented once before (see `deaddrop-alpha.service`'s own "REAL BUG FOUND VIA LIVE TESTING" comment about `cloud-final.service`), but that earlier fix attempt (`After=cloud-init.target` on the systemd unit) created a genuine ordering cycle and had to be reverted, leaving the actual corruption undealt with - "documented as a known, accepted cosmetic artifact rather than chased further" at the time.

### Why the systemd-ordering fix couldn't work, confirmed directly this time rather than just re-asserting it
`systemctl show cloud-init.target -p After` and the same for `cloud-final.service` both confirmed `After=... multi-user.target ...` on real hardware. This service's own `WantedBy=multi-user.target` implicitly orders it *before* multi-user.target (systemd's default for service units pulled in by a target). Depending on either `cloud-init.target` or `cloud-final.service` via `After=` therefore creates the identical cycle no matter which one is picked: this service must precede multi-user.target, which must precede cloud-init.target/cloud-final.service, which this service would need to follow - a contradiction. There genuinely is no systemd-ordering-only fix here.

### Fix: an in-app bounded wait, entirely outside systemd's ordering graph
`client.c` and `server.c` both gained `wait_for_cloud_init_boot_finished()` - polls for cloud-init's own standard completion sentinel, `/var/lib/cloud/instance/boot-finished` (the same file `cloud-init status --wait` itself is built to wait for), bounded to `CLOUD_INIT_WAIT_MAX_MS` (15s) so a genuinely stuck cloud-init can never turn into another silent unrecoverable boot hang - matching this project's now well-established "boots and works with zero manual steps" bound-everything philosophy (`SPLASH_MAX_WAIT_MS`, `CONNECT_TIMEOUT_SECONDS`, ...). Called once, right before `ui_init()` - specifically there and not earlier, since everything before that point either exits immediately on failure (bad args, `wolfSSL_CTX_new` failure) or never touches the console at all; the only thing genuinely at risk of corruption is the ncurses screen `ui_init()` is about to create via `show_splash()`. Because this lives entirely inside the application's own runtime rather than as a systemd dependency, there's no ordering graph for it to conflict with - no cycle possible.

### Verified on a real cold reboot, not just `systemctl restart`
`sudo reboot` on `bravo` (a genuine full OS reboot, not a warm process restart - the earlier boot-hang fix work already established this project's own SD-card/overlay/Tailscale re-init state only truly resets on a real reboot, matching what "unplug/replug" actually exercises). Confirmed via `systemctl show deaddrop-bravo.service -p ActiveEnterTimestamp` vs `cat /var/lib/cloud/instance/boot-finished` that this service's own startup and cloud-init's completion landed within the same second on this particular boot - genuinely close enough that the wait mattered, not a scenario where cloud-init had already finished long before our service even started. `/dev/vcs1` immediately after boot showed a completely clean screen: full quick-help guide, key-share reconstruction, wolfSSL init, and a normal connected session - no stray cloud-init text anywhere in the capture. Deployment to `alpha` pending next time it's reachable (currently out of the dev machine's WiFi range).

## New splash banner: "Twin Nodes," upsized and colored — 2026-08-23

Direct request: pick a cooler design from `docs/splash-banner-options-v2.md` and add color - but first verify, not assume, that both devices are actually still on the smaller 66x20 console constraint that document was written against (the user's own visual impression was that both looked like the larger size).

### Verified directly, not assumed
`stty -F /dev/tty1 size` and `fbset -fb /dev/fb0` run fresh on both real devices: **both** now report `30 100` (30 rows, 100 cols) and a genuine `1280x720` framebuffer - `bravo`'s earlier smaller reading (800x480/66x20, found and parked earlier this session) really was the "DSI auto-detection flakiness" that investigation had already flagged as the likelier explanation over a permanent hardware difference. It resolved itself across this session's several real reboots. The user's visual read was correct.

### Design: Option F ("Twin Nodes"), scaled up and colored
Since neither device is actually constrained to 66x20 anymore, the chosen design was rebuilt larger to make real use of the confirmed 100-col width - two 14-char boxes (`+------------+` / `|   ALPHA    |` / `|   BRAVO    |`), a 34-char `========== L I N K E D ===========` connector between them, 78 cols total (verified with `printf`-guaranteed alignment plus `awk` character counts, not hand-spaced - a first hand-typed draft was off by 3 columns on one line before switching to computed padding). `show_splash()` (`src/ui.c`) now builds this via direct `mvprintw()` calls with explicit column offsets rather than the old `ui_draw_centered()` helper, which only knows how to pad a single centered string inside one box - this layout needs three independently-colored, precisely-positioned pieces on the same row.

**Color**: two new dedicated ncurses color pairs, `CP_NODE_ALPHA` (green) and `CP_NODE_BRAVO` (yellow) - deliberately NOT reusing `CP_OWN_MSG`/`CP_PEER_MSG` (which happen to already be green/yellow for chat-message coloring) to avoid coupling the splash screen's node coloring to an unrelated future chat-color change. The connector uses the existing `CP_ACCENT` (cyan), same pair the subtitle already used.

**Graceful fallback preserved**: the original single-box design didn't get deleted - it's now `show_splash_simple()`, used automatically if `COLS - 2 < TWIN_BLOCK_WIDTH` (62) ever fails on some future, genuinely narrower device. The touch-wait-and-clear tail (`splash_wait_for_dismiss_and_clear()`) was factored out so both layouts share it rather than duplicating that logic.

### Verified on real hardware two ways, not just by reading the code back
1. **Layout**: `/dev/vcs1` immediately after a real service restart on `bravo` showed the exact intended text - both boxes, the connector, and the centered title/subtitle/hint all correctly positioned.
2. **Color, at the byte level**: `/dev/vcsa1` (the attribute-including console device) dumped and decoded by hand, not just trusted from the code - and this surfaced a real, non-obvious finding: the Linux console's raw attribute byte uses **CGA-style** color ordering (0=black, 1=blue, 2=green, 3=cyan, ..., 6=yellow/brown), not the ANSI ordering (0=black, 1=red, 2=green, 3=yellow, ..., 6=cyan) ncurses' `COLOR_*` constants expose to the application. A first read of the raw bytes (`0x06` on what should have been the yellow BRAVO box) looked like a bug under the ANSI assumption - cross-checked against the connector's own attribute byte (`0x03` on its `=` characters, which only makes sense as cyan under CGA ordering, not the "0x06 you'd expect" under ANSI) confirmed it was a reading-convention mismatch, not an actual color bug. Once decoded correctly: `0x02` (green, ALPHA), `0x03` (cyan, connector), `0x06` (yellow, BRAVO) - all three exactly as intended.

Deployed and confirmed healthy (`NRestarts=0`, clean reconnect to peer) on both `bravo` and `alpha`.

### Follow-up: consistent Alpha/Bravo capitalization and identity colors everywhere, plus a real deployment gap found along the way

Direct request: capitalize "Alpha"/"Bravo" everywhere either name is displayed, make Alpha red and Bravo blue consistently across the splash banner, sent/received messages, and the status bar (previously the banner used green/yellow while chat messages used a separate own-vs-peer green/yellow scheme, and the status bar was just plain white-on-blue - three different, uncoordinated color choices for the same two identities).

**What changed:**
- `client.c`/`server.c`: `ui_init()` and `run_symmetric_session()`'s literal name arguments capitalized ("alpha"/"bravo" → "Alpha"/"Bravo") - this one string is the single source of truth every other display site (banner, status bar, message history) derives from.
- `ui.c`: `CP_STATUS` (status bar background) now chosen via `g_self_is_alpha` - red on Alpha, blue on Bravo. `CP_NODE_ALPHA`/`CP_NODE_BRAVO` (the splash banner's two boxes) changed from green/yellow to red/blue to match. The old `CP_OWN_MSG`/`CP_PEER_MSG` (own-vs-peer, green/yellow) pair was retired; `ui_add_history_ex()` now colors a message by the actual sender's NAME via a new `color_pair_for_name()` helper, substituting the real capitalized self-name for session.c's internal `"you"` sentinel at display time.

**A real, separate bug found while investigating why this wasn't visible live already:** both devices' consoles still showed lowercase "connected to bravo"/"alpha" *after* deploying what looked like the right code. Traced to a genuine deployment gap, not a live bug: this capitalization fix had been made once already, earlier in this same overall session, but was **never actually committed to git** - it existed only as an uncommitted local edit on the dev machine. The many `git checkout -- .` cycles used during the WiFi-connectivity troubleshooting (each one a legitimate "discard local drift before pulling" step, done correctly in isolation) had the side effect of silently reverting this specific uncommitted fix back to the last-committed (lowercase) version on both deployed devices, every time. Confirmed directly via `strings ~/deaddrop/build/server | grep bravo` on `alpha` - the compiled binary genuinely had the literal lowercase string baked in, not a stale-binary or caching issue. Fixed by committing all four affected files together this time (`25c641a`) and redeploying via `git pull` (not `scp`) on both devices, so the git history and the running binaries can't silently diverge again.

**A second, genuine (not deployment-related) gap found and fixed along the way:** the "you"→real-name substitution only ever applied to LIVE messages, via `ui_add_history_ex()`. A message replayed from `msglog.txt` on a future boot (`replay_msglog_into_pad_locked()`) writes the log's own already-formatted text straight into the history pad, bypassing that substitution entirely - so a persisted "you: ..." entry would show lowercase "you" forever, regardless of the live-display fix, since the raw text baked into the log file was never converted at write time. Fixed by having `session.c` infer its own capitalized name once per session (`shared_session_ctx.self_label`, computed the same "peer_label != Bravo implies self is Alpha" way `ui.c`'s `g_self_is_alpha` already is) and pass that directly to `msglog_append()`/`ui_add_history()` instead of the `"you"` sentinel - so what's written to disk already has the real name in it.

**Verified live on real hardware, both fixes, not just re-read from the code:**
- Status bar: `alpha` showed `DEADDROP :: Connected to Bravo`, `bravo` showed `DEADDROP :: Connected to Alpha` (both capitalized) after redeployment.
- Live message coloring/naming: sent a real message from `bravo` via a synthetic keyboard tool (`/dev/uinput`); `alpha`'s screen showed `Bravo: ...` (capitalized) rather than the previous `bravo: ...`.
- The persisted-replay gap specifically: had `alpha` send its own message ("hi", via a synthetic keyboard tool), confirmed it displayed live as `Alpha: hi`, then did a REAL `systemctl restart` (not just a code check) and confirmed the replayed history on the fresh boot showed `Alpha: hi` again - not `you: hi` as it would have before the `self_label` fix.
- Full regression unaffected: both services `active`, `NRestarts=0`, git checkouts on both devices confirmed matching `25c641a` exactly.

Test tooling (`build/synth_msg*.c`, throwaway `/dev/uinput` keyboard-injection tools) and the stale pre-fix `message_log.txt` on both devices (containing the old lowercase-era test entries) removed before final verification.

### Follow-up, same day: a real status-bar color inversion found via direct hardware feedback, plus timestamp/error color collisions and OLED capitalization

Direct request, given as a concrete worked example rather than an abstract description: the status bar ("header") should show the color of the device it's running ON (self-color), not the device it's connected TO (peer-color) - e.g. `DEADDROP :: Connected to Bravo`, shown on alpha's own screen, should be red (alpha's own color), which is in fact exactly what the existing `CP_STATUS` design already intended (`g_self_is_alpha ? RED : BLUE`).

**Verifying that concrete example against real hardware (rather than trusting the code) found a genuine, previously-undetected bug:** decoding the status bar's actual attribute bytes via `/dev/vcsa1` (not just its text, which had already been checked and looked fine) showed `alpha` rendering BLUE (`0x17`) and `bravo` rendering RED (`0x47`) - exactly backwards. Root cause: `g_self_is_alpha`'s computation read `strcmp(peer_label, "Bravo") != 0` - i.e. "self is Alpha unless the peer is Bravo" - which inverts the actual relationship (if the PEER is Bravo, THIS device is talking to Bravo, which makes IT Alpha - the check needed to be `== 0`). This bug had been live since the identity-coloring work went in, but nothing had caught it: every other verification that day (message coloring, capitalization, persisted-replay) happened to go through `session.c`'s separately, independently, and correctly computed `self_label` - only `CP_STATUS` (the one thing actually driven by `g_self_is_alpha`) was wrong. A worthwhile lesson on its own: two independent computations of "the same" fact (`g_self_is_alpha` in `ui.c`, `self_label` in `session.c`) can silently disagree, and testing one doesn't prove the other.

Fixed to `== 0`, matching `self_label`'s already-correct logic exactly. **Verified live**: re-decoded `/dev/vcsa1` on both devices after the fix - `alpha` now `0x47` (red), `bravo` now `0x17` (blue), both correct.

**Also fixed, both explicitly requested and caught by inspection while already in this code:**
- `CP_TIMESTAMP` (was blue) and `CP_ERROR` (was red) directly collided with `CP_NODE_BRAVO`/`CP_NODE_ALPHA`'s new blue/red identity colors from the same day's earlier work - a timestamp prefix or an error line was no longer visually distinguishable from a message actually attributed to that device, defeating the point of identity coloring. Moved to yellow and magenta respectively (both otherwise unclaimed in the 8-color palette). `CP_LOCKED` deliberately left red - it's a full-screen overlay word, never adjacent to a `Name: text` line, so it doesn't carry the same misattribution risk. Verified live: decoded a real timestamped history line's attribute byte on `alpha` (`0x6` = yellow, correct); the equivalent live check for `CP_ERROR` was abandoned after the same `/dev/uinput` synthetic-keyboard tool's known character-garbling quirk (documented several times earlier this session) repeatedly mangled the `/send` command text into plain chat messages instead of triggering the intended failure path - not worth fighting further given `CP_ERROR` uses the exact same mechanism (`init_pair` + a boolean color-pair selection) already proven correct for `CP_TIMESTAMP` moments earlier.
- The banner itself was explicitly confirmed correct and left untouched - decoded before making any change and found ALPHA's box already red (`0x4`) and BRAVO's already blue (`0x1`) on both devices, exactly as intended; the user confirmed the fixed Alpha-left/Bravo-right, absolute-not-self-relative layout is the desired design, not a bug.
- `client.c`/`server.c`'s `"DeadDrop alpha"`/`"DeadDrop bravo"` OLED role-label strings capitalized to match every other display site. The one other OLED text site using a name (`session.c`'s `"From %s:"` incoming-message preview) was already correct, since it's driven by the already-capitalized `peer_label`.

Full regression suite (`test_aes128`, `test_hmac`, `test_sha256`, `test_revocation`, `test_message`) re-run clean on `alpha` after all of the above - 5/5. Both services confirmed `active`, `NRestarts=0`, git checkouts matching `b255775` on both devices.

## Security audit, 2026-08-23 - "how secure is this actually" pass

Direct request to move into real security testing on top of the functional work: network exposure, TLS/cert enforcement, privilege scoping, key material handling, memory safety, and app-layer attack surface.

### Finding #1 (real, confirmed, fixed): the main mTLS listener was reachable from the local WiFi network, not just the tailnet

**How it was found:** `sudo ss -tlnp` on `alpha` during the audit's very first check showed `0.0.0.0:4433` - the main DeadDrop protocol port bound to ALL interfaces, not scoped to Tailscale. `docs/tailscale-acl.json`'s entire purpose is to restrict `tag:deaddrop-client -> tag:deaddrop-server:4433` traffic to the tailnet - but that ACL only governs traffic Tailscale itself routes. A socket bound to `INADDR_ANY` is reachable by anyone who can reach the device's real LAN IP directly, completely bypassing the ACL. `keyshare.c`'s own share-listener (port 4434) already had exactly the right pattern for this - a `get_own_tailscale_ip()` helper with its own comment explicitly stating "deliberately never INADDR_ANY" - but `server.c`'s main listener never got the same treatment, a genuine inconsistency between two parts of the same codebase, not a deliberate choice.

**Impact, honestly assessed:** mTLS still rejected any unauthenticated connection attempt - this was never a full compromise. But it meant the entire TLS/TCP handshake surface (and everything upstream of certificate validation - connection handling, the accept loop, wolfSSL's own parsing of the initial handshake bytes) was reachable by anyone on the same network, which on a managed/shared property network like "The Arrow" (confirmed earlier this project to be a genuine multi-AP, multi-tenant deployment) is a meaningfully larger population than "just the paired device."

**Fix:** added `keyshare_get_own_tailscale_ip()` to `keyshare.h`/`keyshare.c` (a thin public wrapper around the existing internal `get_own_tailscale_ip()` the share-listener already used), and changed `server.c`'s `bind()` to use it instead of `INADDR_ANY` on Linux (Windows dev-machine builds keep `INADDR_ANY` unconditionally - there's no Tailscale plumbing there and dev-machine testing already uses a direct loopback PEM-file path instead of this whole scheme). Deliberately **fail-closed**: if the Tailscale IP can't be determined, the server refuses to start rather than silently falling back to an unscoped bind - by the time this code runs, `keyshare_reconstruct()` has already succeeded moments earlier in the same startup, which itself required a working Tailscale connection, so this realistically never triggers; a loud refusal is the right response for a security-relevant network binding regardless.

**Verified two ways, not just by reading the code:**
1. `sudo ss -tnp` after the fix and a restart showed the real, live connection as `ESTAB 100.124.177.123:4433 <-> 100.103.41.44:59220` - `bravo` reconnected cleanly (it already targets alpha's Tailscale IP directly, `-h 100.124.177.123`, so this required zero client-side changes), and the accepted socket's local address confirms `listen_sock` itself was genuinely bound to the Tailscale IP (an accepted connection always inherits its listener's local address).
2. **The actual, direct proof**: with the dev machine and `alpha` both genuinely on the same WiFi network ("The Arrow") at the time, `Test-NetConnection -ComputerName 100.64.8.61 -Port 4433` (alpha's real LAN IP) returned **`False`** - unreachable - while `Test-NetConnection -ComputerName 100.124.177.123 -Port 4433` (alpha's Tailscale IP) returned **`True`** on the very same network, at the very same moment. Not a simulated test - a real second device on the real network the property actually uses, genuinely unable to reach the port anymore.

Full regression suite re-run clean (5/5) after the fix.

### Finding #2: static analysis pass (cppcheck) - clean

`cppcheck --enable=warning,performance,portability --inconclusive --std=c99` against all 20 files in `src/` (both `_WIN32` and `__linux__` code paths, since cppcheck evaluates both branches of the project's portability shims) came back genuinely clean - zero warning/performance/portability/style findings. The only two items flagged at all were `uninitvar` "errors" on `localtime_s(&tm_now, &now)` in `msglog.c` and `ui.c` - confirmed as a known cppcheck false positive, not a real bug: `localtime_s`'s first parameter is OUTPUT-only (the function WRITES the struct, per its actual C11 Annex K contract), and cppcheck's default function-behavior library doesn't have accurate output-vs-input metadata for this specific Windows CRT function. Both real call sites already correctly `strftime()` the filled-in struct immediately afterward - the standard, safe idiom, not a genuine read-before-write.

### Finding #3 (real, confirmed, fixed): `connor` had unscoped passwordless root, though the app itself only ever needs three specific `nmcli` commands

The deployed app (both `server`/`client` binaries) already runs as the unprivileged `connor` user (`User=connor`/`Group=connor` in both unit files) - a correct baseline design. But `connor` has `ALL=(ALL) NOPASSWD:ALL` via `/etc/sudoers.d/010-connor-nopasswd` (set up back in Week 4 Day 1, for a different reason - remote deployment convenience over SSH, since asking for the user's real account password is against this project's own standing rule). This means the "runs as non-root" design provided **zero actual privilege separation**: any bug in the app reachable by an authenticated peer (a parsing bug in the message/keyshare/WiFi-scan-output handling, say) that achieved even basic code execution as `connor` could trivially become full root with one `sudo` call - no exploit chain needed, no privilege-escalation vulnerability required, just the ambient grant already sitting there.

**Enumerated exactly what the app itself ever actually invokes via `sudo`** (grepped every `"sudo"` argv literal in `wifi.c` - the only file that ever shells out with elevation): `nmcli device wifi rescan` (fixed, no arguments) and `nmcli device wifi connect <ssid> [password <password>]` (SSID/password are user-supplied at the WiFi setup screen, but the command structure is fixed). Nothing else in the codebase invokes `sudo` for anything, ever, at runtime.

**Added a genuinely least-privilege rule** (`docs/020-deaddrop-wifi-sudoers`, installed as `/etc/sudoers.d/020-deaddrop-wifi`, mode 440 root:root, `visudo -c` clean on both devices) using a `Cmnd_Alias` scoped to exactly those two command patterns - verified via `sudo -l -U connor`, which confirmed the rule resolves to precisely `(root) NOPASSWD: /usr/bin/nmcli device wifi rescan, /usr/bin/nmcli device wifi connect *` and nothing broader.

**The broader question - narrow or remove the existing unscoped `010` rule - was put to the user directly**, since it's a genuine, live tradeoff between real hardening and this project's own established remote-deployment workflow (which has relied on that broad grant for essentially the entire project, given the no-password-ever rule leaves no other path to root for me). **Decision: keep the broad rule for now**, explicitly as a documented, deliberate operator-convenience gap rather than an unexamined oversight - not something the app itself needs (the new `020` rule proves that), but something *I* still need to keep doing remote systemctl/package/log work without the user needing to be present to type commands or a password. Worth revisiting once the project reaches a more finished, hands-off state.

### Finding #4: systemd sandboxing - a carefully scoped subset added, several aggressive directives deliberately excluded and documented why

Neither `deaddrop-alpha.service` nor `deaddrop-bravo.service` had ANY systemd sandboxing directives before this pass - just `User=`/`Group=` and the TTY-ownership settings. Real, additional hardening is available here independent of the sudo question above (sandboxing reduces what a compromised process can do even before/without ever touching `sudo`), but this needed real care: the app has several genuine, non-negotiable runtime dependencies that many of the "usual" hardening templates would silently break -  `sudo`-invoked `nmcli` (itself talking to NetworkManager over D-Bus/`AF_UNIX`), a real owned TTY (`/dev/tty1`), `/dev/i2c-1` for the OLED/RGB hardware, `/dev/input/eventN` for touch, and `espeak-ng` as a forked child process for TTS.

**Deliberately excluded, each with a real, specific reason** (documented in both unit files' own comments, not just this log): `NoNewPrivileges` (would block `execve()` of any setuid binary, including `sudo` itself - the WiFi setup screen would silently stop working), `RestrictAddressFamilies` (nmcli's D-Bus traffic, inherited by the same seccomp filter as a child process, needs `AF_UNIX`; enumerating every family every child genuinely needs is real work not to rush through in one pass against the user's only two physical field devices), `CapabilityBoundingSet` (an empty/reduced bounding set would neuter `sudo` too - a setuid-root exec only grants capabilities up to the parent's own bounding set at exec time), and `ProtectSystem=strict`/`PrivateDevices`/`PrivateTmp`/`SystemCallFilter` (each has plausible interaction with the hardware access or the `espeak-ng` subprocess that would need much more verification time than a single pass safely allows, given the very real risk of silently bricking a field device with no easy remote recovery path if gotten wrong).

**What WAS added**, checked in advance against every one of those real runtime needs and found to have no plausible interaction: `ProtectKernelTunables`, `ProtectKernelModules`, `ProtectKernelLogs`, `ProtectControlGroups`, `ProtectClock`, `ProtectHostname`, `RestrictNamespaces`, `RestrictSUIDSGID`, `LockPersonality`, `MemoryDenyWriteExecute`, `RestrictRealtime` (all `true`), plus `ProtectHome=read-only` with `ReadWritePaths=/home/connor/.deaddrop` - verified precisely by grepping every `fopen(..., "w"/"a")` call site in the entire codebase (`msglog.c`'s append/rewrite, `lock.c`'s PIN-hash write, `session.c`'s received-file save) and confirming all three resolve under exactly that one directory, nothing else.

**Tested on `bravo` first** (client role - a mistake there doesn't take down the whole system the way a mistake on `alpha`, the server, would), with real functional verification of the two highest-risk paths, not just "it starts without erroring":
- `sudo ./build/synth_ctrlw2` (a throwaway `/dev/uinput` Ctrl+W injector) triggered a genuine WiFi scan under the new sandboxing - real results (`The Arrow 802.1X (secured)`, `The Arrow (open)`) rendered correctly, proving `sudo` + `nmcli` + D-Bus/`AF_UNIX` all still work.
- A real message typed and sent under the new sandboxing appeared correctly in `~/.deaddrop/message_log.txt` on `bravo` AND was received and logged on `alpha` (`[...] Bravo: sec`) - full round-trip, proving `ReadWritePaths` is correctly scoped and the rest of `ProtectHome=read-only` doesn't block the reads (`~/pki`, `~/keyshare`) the app also depends on.
- One real scare during this process turned out to be nothing: a `Main process exited, code=exited, status=1/FAILURE` on the very first restart with the new unit looked like a hardening-caused crash - but the identical pattern was then confirmed on `alpha`'s completely UNMODIFIED unit file too (this app has never had a graceful SIGTERM handler; every `systemctl restart` has always logged this way, throughout the whole project - a pre-existing, already-known, already-accepted characteristic, not a new regression).

Applied to `alpha` only after `bravo`'s full verification passed. Both devices confirmed `active`, `NRestarts=0`, both showing genuine live connections (`Connected to Bravo`/`Connected to Alpha`) after the change. Full regression suite re-run clean (5/5) on `alpha` after deployment.

### Finding #5 (real, confirmed, fixed): received files (attacker-influenceable content) weren't getting the same explicit permission treatment as the PIN hash and message log

While re-checking the file-write sites already enumerated for Finding #4's `ReadWritePaths` scoping, noticed `lock.c`'s `pin_hash` write and `msglog.c`'s `message_log.txt` write both explicitly `chmod(path, 0600)` right after `fclose()` - a deliberate "don't trust ambient umask" precedent, each with its own comment saying so. `session.c`'s received-file save (the `DD_MSG_FILE` handler, content that arrived over the wire from the peer - genuinely attacker-influenceable, unlike the other two) had no such call, just a bare `fopen(path, "wb")` left to whatever default mode + umask the process happens to run under.

**Confirmed this was a real gap, not a theoretical one**, by directly probing the actual target mount as the actual `connor` user rather than assuming: created a file the same way (default `fopen`/redirect semantics) in `~/.deaddrop/received/` on `alpha`'s real ext4 persist-overlay mount - landed `664` (group- and world-readable), not `600`. The parent directory (`0700`, set via this codebase's own `MKDIR` macro) already blocks any other user from traversing in on this specific single-user device, so the practical exposure today is narrow - but it's exactly the kind of "safe by accident, not by design" gap this audit exists to catch, and it's inconsistent with the precedent already established twice in the same codebase for less attacker-influenced files.

**Fix:** added the identical `#ifndef _WIN32 chmod(path, 0600) #endif` pattern to `session.c`'s received-file write path, matching `lock.c`/`msglog.c` exactly, with a comment cross-referencing both and explaining why umask shouldn't be trusted here specifically (the systemd units set no `UMask=`, so it would otherwise silently inherit whatever PID1's default happens to be).

**Verified for real:**
- Compiled clean (`-Wall -Wextra`, zero warnings) on the dev machine and, separately, natively on both Pis after `git pull`.
- Re-ran the exact same real-mount probe (create default-mode file, `chmod 0600`, `stat`) on `alpha`'s live `~/.deaddrop/received/` directory - `664` before, `600` after, confirming the fix's mechanism works correctly on the actual production filesystem/mount, not just in theory.
- **A second, unrelated real bug found and fixed along the way**: `alpha` had a stray untracked `src/keyshare.h` - byte-identical to `include/keyshare.h` at the time, but since every source file's `#include "keyshare.h"` is quote-form (searches the including file's own directory, `src/`, before falling back to the `-Iinclude` search path), this file was silently *shadowing* the real header for every build on that device. Harmless right then only because the two happened to be identical; the next edit to `include/keyshare.h` alone (without also touching the stray copy) would have silently failed to take effect on `alpha`'s local builds - the same silent-divergence bug class already hit twice earlier this project (the reverted-by-`git checkout`-uncommitted-fix incident, and this same finding's own discovery that `alpha` had three files locally modified but never committed, byte-for-byte identical to Finding #1's already-pushed fix). Removed the stray file, reconciled `alpha`'s working tree with `git checkout --` (confirmed identical first, not blind-discarded), then `git pull` cleanly fast-forwarded.
- Full rebuild (`server`+`client`, all object files) and full 5/5 regression suite re-run clean on both devices after the pull. Both services restarted, confirmed `active`, and re-verified end-to-end: `alpha`'s listener still correctly bound to `100.124.177.123:4433` (not `0.0.0.0`) with a live `ESTABLISHED` session to `bravo`, status bar correctly reading `Connected to Bravo` in red on `alpha`'s own screen - the entire audit's cumulative state (Findings #1, #3, #4, #5, plus the earlier color/capitalization fixes) all confirmed still intact together, not just each fix in isolation.

### Post-rebuild re-verification of Finding #1/#3/#4 together, plus the sandboxed WiFi-scan and message-round-trip checks re-run fresh

Since Finding #5's rebuild restarted both services, re-ran the highest-value functional checks from Findings #1 and #4 fresh, live, on the currently-running binaries rather than assuming they still held from before the rebuild:
- **Reconnection**: both consoles checked directly (`/dev/vcs1`) - `alpha` showing `Connected to Bravo`, `bravo` showing `Connected to Alpha`, each in its own correct self-color. Journals on both showed only the already-known, already-documented no-SIGTERM-handler exit-1-on-restart pattern - no real connect failures, retries, refusals, or timeouts.
- **Sandboxed WiFi scan, re-run**: a corrected `synth_ctrlw` chord on `bravo` under the current (already-applied) hardened unit triggered a real scan - `The Arrow (open)`, `The Arrow 802.1X (secured)`, and a newly-in-range `Roaming Beef Boy (secured)` all rendered correctly, confirming `sudo`+`nmcli`+D-Bus/`AF_UNIX` still work post-rebuild. Cancelled cleanly with a second Ctrl+W.
- **Sandboxed message round-trip, re-run with the corrected synthetic-keyboard tool**: TESTING.md's own earlier root-cause fix for this tool category (Linux keycodes aren't alphabetically contiguous - `KEY_P`=25 jumps straight to `KEY_A`=30 - so `KEY_A + (letter-'A')` arithmetic silently produces wrong keys) was rebuilt as a real per-character lookup table rather than trusted from memory. Result: a full test message (`sandboxaudittest`) typed and sent from `bravo` arrived with **zero garbling** - the first fully clean synthetic-keyboard text entry this project has had, confirming that earlier fix was genuinely correct, not just theorized. Delivered live to `alpha`'s screen as `Bravo: sandboxaudittest` and persisted to `message_log.txt` on **both** devices with correct `600` permissions. Test tooling and the test message itself removed from both logs afterward, matching this project's own artifact-cleanup discipline.
- Confirmed `alpha`'s live installed unit (`/etc/systemd/system/deaddrop-alpha.service`) already carries the Finding #4 sandboxing directives (`ProtectHome`, `RestrictNamespaces`, `MemoryDenyWriteExecute`, etc.) - it did not need reapplying.

### Finding #6 (real, confirmed, low-severity given the existing threat model): the PIN lock's wrong-attempt rate-limiting delay is in-process memory only and resets on any restart

`lock.c` itself has zero attempt-limiting - `lock_check_pin()` is a pure hash comparison with no counter, no lockout, no persisted state. The actual rate limiting lives entirely in `ui.c`: a file-scope `static int wrong_attempt_count` drives a linear, 5-second-capped delay (`ui.c`'s own design comment, written when this feature was built, is explicit that this is a deliberate "speed bump," not a hard security boundary - the documented threat model for this screen is "reaching it at all already requires physical possession of a powered, connected device").

**Verified empirically, not just read from the code**, using the same "seed state directly, bypass UI text entry" technique as the project's earlier lock-screen tap-gesture test (`lock_set_pin()` called directly from a throwaway harness) plus a corrected synthetic-keyboard tool, on `bravo`:
1. Seeded a known test PIN, restarted so `ui.c`'s cached `pin_configured` flag picked it up, confirmed boot-to-locked behavior.
2. First wrong attempt: `Wrong PIN (wait 1s before next try)`. Second wrong attempt: `wait 2s` - confirmed the delay genuinely ramps as designed.
3. **The actual test**: restarted the service mid-ramp (`systemctl restart`, the same action `Restart=always` performs automatically after any crash/kill, no special privilege needed beyond what already triggers a normal restart), then sent one more wrong attempt. Result: `wait 1s` again - not `3s` or higher. **Confirmed: the counter does not survive a restart**, exactly as the in-memory-only variable declaration implied.
4. Correct-PIN unlock re-confirmed working immediately afterward (`13579` accepted, returned to `UI_MODE_NORMAL` cleanly).
5. Cleaned up: removed the seeded PIN file, throwaway test tooling on both the Pi and the dev machine, and restarted `bravo` back to its original no-PIN baseline - confirmed clean (`active`, no `pin_hash`, reconnected to `alpha`) afterward.

**Severity assessed honestly, not inflated**: this doesn't expand the threat model `ui.c`'s own design already accepted - an attacker who can force a process restart to reset the delay almost certainly already has the physical access (a power cycle, a reset button, physical console access) that the lock screen's whole premise assumes as a given. It removes a "speed bump" against a scripted brute-force via an *already-attached* keyboard, not a real security boundary; SHA-256 with a salt (no attempt-limiting at all needed to still resist offline attack, since the hash file itself is `0600`-protected and never leaves the device). Documented rather than fixed for now - a real fix (persisting the counter/next-allowed-time to the same `0600`-protected file the hash lives in) is a reasonable, low-effort future improvement, but not urgent given the existing physical-access threat model already covers the realistic exploitation path.

### Fix applied: Finding #6's PIN lockout deadline is now persisted, closing the gap instead of just documenting it

Per direct request to patch everything the audit found rather than leave the low-severity items as notes: added `lock_get/set/clear_next_allowed_time()` (`lock.c`, a separate `0600` file, `~/.deaddrop/lock_ratelimit`, deliberately not appended to `pin_hash`'s own fixed format) and wired them into `ui.c` - loaded once at `ui_init()`, persisted on every wrong attempt, cleared on a successful unlock. Deliberately scoped to just the deadline, not the full ramping counter (`wrong_attempt_count` itself still resets to 0 in memory on every restart) - persisting only the timestamp still closes the actual exploitable gap (a restart can no longer make attempts arrive faster than whatever delay is already in effect), while staying a small, easy-to-reason-about change rather than a bigger rework of the ramping logic itself.

**Verified for real, twice, on `bravo`:**
1. Re-ran the exact same live restart-mid-lockout test as the original Finding #6 discovery (seed a test PIN, ramp the delay via real wrong attempts, restart mid-ramp) - timing across sequential SSH round-trips proved too tight to reliably catch a 1-5 second window in the act (several attempts landed just after the deadline had already naturally elapsed, which is itself correct behavior for an elapsed deadline, just not proof the persistence worked).
2. Switched to a deterministic version of the same test instead of fighting real-time races: wrote a known, generously-future deadline (now + 180s) directly to the persisted file, restarted, waited for the lock screen, and attempted a PIN. Result: `Please wait 116 more second(s).` - the persisted deadline was correctly loaded at boot and enforced, definitively confirming the fix works, not just that the code compiles.
3. Cleaned up (removed the test PIN and ratelimit file, restarted `bravo` back to its clean baseline) afterward.

Rebuilt and redeployed to both devices (`lock.c`, `ui.c`, plus the already-pending `LISTEN_BACKLOG` and `wifi.c` changes below), full 5/5 regression suite clean on both, both services confirmed `active` and reconnected after the deploy.

### Finding #7 (real, confirmed, fixed): re-running the TLS negative-path tests live surfaced a genuine availability gap - `LISTEN_BACKLOG` of 1

Re-verifying the four Week 2 Day 5 negative tests against the *current, live, field-deployed* `alpha` (not a fresh standalone instance) was the original plan, using a new throwaway harness (`tls_negtest.c` - a minimal, non-interactive wolfSSL client mirroring `client.c`'s own CTX setup, since driving the full ncurses `client.c` non-interactively isn't practical) reaching `alpha` directly over Tailscale from the dev machine (confirmed reachable first via `Test-NetConnection`).

**The first connection attempt hung indefinitely** - not a bug in the negative-path logic itself, but a real, previously-undiscovered gap it happened to expose: `server.c` handles exactly one connection for the *entire duration* of a session (a permanent `for(;;) { accept(); ...; run_symmetric_session(); }` loop, `accept()` only called again once the current session fully ends - normal and correct for this app's design, since two paired devices are meant to hold one long-lived session for hours or days at a time). `CONN_TIMEOUT_SECONDS` only starts counting once `accept()` actually returns a socket - a connection still waiting in the kernel's own accept queue, never yet handed to the app, is invisible to every existing timeout/watchdog. With `LISTEN_BACKLOG` set to 1, two connection attempts during this exact test session were enough to overflow it - confirmed directly via `ss -tn` showing `Recv-Q 2` against a `Send-Q` (backlog) of `1` on the live listening socket, not inferred.

**Real impact**: while `alpha` is legitimately busy with `bravo`'s session (its normal, constant operating state), any additional connection attempt - a probe, noise, or critically, `bravo`'s own reconnect attempt after a real network blip - could be silently dropped at the TCP level with zero application-level log line, since the app never even sees a connection it never `accept()`s.

**Fix:** raised `LISTEN_BACKLOG` from 1 to 8 - small and still deliberately bounded, enough to absorb a genuine reconnect race or a short burst of probes without changing anything about mTLS rejecting every connection beyond the first, which still happens exactly as before, immediately after `accept()`. Also checked `keyshare.c`'s own listener (same `backlog=1` pattern) for the identical risk and found it materially different, not a bug: its `listener_thread_main()` uses a bounded `poll()` loop and handles each connection near-instantly (single read/write/close), returning to `accept()` again within milliseconds every cycle - not "blocked for hours on one session" like `server.c`'s main listener - so left unchanged rather than patched by blind analogy.

**To actually run the negative tests without disrupting the live paired devices** (stopping `bravo` to free `alpha`'s single accept slot turned out to also block `alpha`'s own boot - it needs `bravo`'s keyshare listener up to complete its own `keyshare_reconstruct()` step first, an architectural reality of the mutual-custody design, not a bug), pivoted to the same methodology the *original* Week 2 Day 5 tests actually used: a standalone `server.exe` built fresh on the dev machine from the current `server.c` (same source, including the `LISTEN_BACKLOG` fix), run with `-k` (plain PEM key, dev-machine testing mode) against this project's real production CA/cert/key material and the real, still-surviving `throwaway-pki/` negative-test certs from Week 2.

**All five re-confirmed against the current build, server-side verbatim:**
| Test | Server log | Pass/fail |
|---|---|---|
| No client cert | `Handshake failed: peer did not return a certificate` | **PASS** |
| Wrong CA | `Verify callback: standard cert-chain verification failed.` / `certificate verify failed` | **PASS** |
| Expired cert | `Verify callback: standard cert-chain verification failed.` / `ASN date error, current date is after expiration` | **PASS** |
| Revoked cert (real serial `68A7E95F...`) | `Verify callback: serial ... is REVOKED - rejecting.` / `verify problem on certificate` | **PASS** |
| Happy path (valid, non-revoked) | `mTLS handshake succeeded` / `Connected to Bravo` | **PASS** |

One honest methodology note: the throwaway client harness's own detection of the "no client cert" rejection was unreliable (reported `mTLS handshake succeeded` client-side, the same TLS-1.3-half-open-completion blind spot Week 2 Day 5 already documented and fixed in the *real* `client.c` - this quick harness didn't get the same treatment since the server-side log is definitive proof either way). Treated the server's own verbatim log as authoritative, matching the original test table's own structure (server result as the primary field, client result as secondary/informational).

### Finding #8: `keyshare.c`'s wire protocol reviewed, found structurally safe by design - no fuzz harness built

`fetch_remote_share()` reads exactly `KEYSHARE_LEN` fixed bytes with no attacker-controlled length field at all (a malicious peer sending more just leaves the excess unread; sending less/garbage just produces a wrong-but-not-crashing reconstructed key, caught downstream when decryption fails) - there's no parsing surface for a memory-safety bug to live in. `verify_peer_identity()` does parse variable text (`tailscale whois` output) but into a fixed, always-NUL-terminated buffer with explicit bounds on every `strncmp`/loop, and the comparison lengths come from a local compile-time constant (`"deaddrop-alpha"`/`"deaddrop-bravo"`), never attacker-supplied data. Reviewed line-by-line rather than fuzzed - building a synthetic fuzz harness for code with no attacker-controlled length or format string to mutate would have had genuinely low expected value.

### Finding #9: `wifi.c`'s SSID parser - the one genuinely unauthenticated attacker-influenceable parsing surface in this codebase - extracted, fuzzed, clean

Unlike `message.c` (gated by a completed mTLS handshake) or `keyshare.c` (gated by `verify_peer_identity()`, Finding #8 above), a WiFi SSID is broadcast over the air by anyone within radio range with zero authentication of any kind, and `wifi_scan()`/`wifi_get_link_info()` both parse it out of `nmcli`'s colon-escaped terse output. Found the exact same colon-escape-aware parsing loop duplicated byte-for-byte in both functions - extracted into a single shared `parse_escaped_ssid_field()` helper first (a real, if minor, finding on its own: the same silent-divergence risk this project has hit multiple times already, a future fix to one copy not automatically reaching the other), then added `tests/fuzz_wifi.c` against it, `#include`-ing `src/wifi.c` directly so the harness fuzzes the *actual* production parser rather than a hand-copied reimplementation that could silently drift from it.

Three strategies, run under GCC's native Linux ASan+UBSan on `bravo` (no MSYS2/Clang detour needed here, unlike the Windows dev-machine sanitizer work - Linux GCC has always shipped mature sanitizer support): 500,000 pure-random valid C strings, 500,000 structurally-aware mutations biased toward the two bytes this parser treats specially (`:` and `\`, seeded from base strings like an all-colons line, a trailing dangling backslash, stacked escapes), and a deterministic fixed pass covering every truncation-boundary `out_size` against seven hand-picked edge cases (empty line, lone trailing backslash, all-backslash line, etc.). Every run checked two real invariants directly (not just "didn't crash"): the output is always NUL-terminated within the caller's buffer size, and the cursor never leaves `[line, line + strlen(line)]`.

**Result: `FUZZING COMPLETE, NO CRASHES` - zero ASan/UBSan reports across 1,000,000+ iterations plus the full deterministic edge-case pass, in 2.5 seconds.**

### Finding #10: the `/destroy` emergency-wipe path reviewed, found to add no new parsing surface beyond what's already fuzz-tested

The wire-level `DD_MSG_DESTROY` message carries an empty body - no attacker-controlled content at all beyond the message-type byte itself, which `dd_try_parse_message()` already handles generically and was already exhaustively fuzzed (Week 3 Day 5, 1,000,000+ iterations across multiple base-message sizes). `session_perform_local_destroy()`'s actual wipe logic (`wipe_directory_contents()`) only ever enumerates and deletes files already on this device's own filesystem via `opendir`/`readdir`, using `snprintf` with an explicit bounds check before every `remove()` call - no network-supplied filename or length ever reaches it. The one genuine design question - a successfully mTLS-authenticated peer can trigger a full remote wipe with a single empty message, no local confirmation on the receiving end - was assessed as consistent with, not a deviation from, this project's own stated two-device mutual-trust security model (a peer that can complete mTLS has already passed the same trust bar needed to read/send messages as that device; remote-destroy isn't a materially different capability than that).

### Audit scope: complete

All six originally-proposed areas (network exposure, static analysis, privilege scoping, systemd sandboxing, file permissions, PIN brute-force resistance) plus the two deferred items from the previous pass (TLS negative-path re-verification, protocol-surface fuzzing) now have a real finding-and-fix, a clean verified pass, or an honestly-scoped "reviewed, no meaningful additional surface" note - ten findings total (#1-#10), seven with real fixes deployed and verified on both real devices, one clean fuzz pass, two "reviewed and found already safe by design." Nothing was left as "documented but not fixed" by the end of this pass.
