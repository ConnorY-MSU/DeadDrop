...
- `docs/provision-permissions.sh` re-run on both devices - `chown`/`chmod` applied cleanly to `~/pki`, `~/keyshare`, `~/.securelink` on both, no ownership drift found this time (unlike the real bug this script was written to catch earlier in the project).
- Both `securelink-alpha.service`/`securelink-bravo.service` confirmed `active (running)` on the latest build after every rebuild in this section.

### Real bug found and fixed: scrollback line-count didn't account for wrapped lines
Before starting user testing, both Pis were reset for a genuine out-of-box first-boot experience: `~/.securelink/message_log.txt`, `~/.securelink/pin_hash`, and `~/.securelink/received/*` deleted on both (leaving `~/pki`/`~/keyshare`/network config untouched - those are infrastructure, not chat state), services restarted. Dumping the real physical console (`/dev/vcs1`) on both to confirm - `alpha` looked correct (fresh quick-help guide, "No PIN set" prompt, no stale history), but **`bravo`'s screen had already scrolled the quick-help guide off-screen**, despite having nowhere near a viewport's worth of genuinely separate lines to justify it.
...
Direct user report from an actual unplug/replug power cycle (not a `sudo reboot`, a real cold boot): "the boot sequence overlaps onto the banner." This is the same underlying cause this project already found and documented once before (see `securelink-alpha.service`'s own "REAL BUG FOUND VIA LIVE TESTING" comment about `cloud-final.service`), but that earlier fix attempt (`After=cloud-init.target` on the systemd unit) created a genuine ordering cycle and had to be reverted, leaving the actual corruption undealt with - "documented as a known, accepted cosmetic artifact rather than chased further" at the time.
...
- Status bar: `alpha` showed `SECURELINK :: Connected to Bravo`, `bravo` showed `SECURELINK :: Connected to Alpha` (both capitalized) after redeployment.
...
Direct request, given as a concrete worked example rather than an abstract description: the status bar ("header") should show the color of the device it's running ON (self-color), not the device it's connected TO (peer-color) - e.g. `SECURELINK :: Connected to Bravo`, shown on alpha's own screen, should be red (alpha's own color), which is in fact exactly what the existing `CP_STATUS` design already intended (`g_self_is_alpha ? RED : BLUE`).
...
- `client.c`/`server.c`'s `"SecureLink alpha"`/`"SecureLink bravo"` OLED role-label strings capitalized to match every other display site. The one other OLED text site using a name (`session.c`'s `"From %s:"` incoming-message preview) was already correct, since it's driven by the already-capitalized `peer_label`.
...
**How it was found:** `sudo ss -tlnp` on `alpha` during the audit's very first check showed `0.0.0.0:4433` - the main SecureLink protocol port bound to ALL interfaces, not scoped to Tailscale. `docs/tailscale-acl.json`'s entire purpose is to restrict `tag:securelink-client -> tag:securelink-server:4433` traffic to the tailnet - but that ACL only governs traffic Tailscale itself routes. A socket bound to `INADDR_ANY` is reachable by anyone who can reach the device's real LAN IP directly, completely bypassing the ACL. `keyshare.c`'s own share-listener (port 4434) already had exactly the right pattern for this - a `get_own_tailscale_ip()` helper with its own comment explicitly stating "deliberately never INADDR_ANY" - but `server.c`'s main listener never got the same treatment, a genuine inconsistency between two parts of the same codebase, not a deliberate choice.
...
- A real message typed and sent under the new sandboxing appeared correctly in `~/.securelink/message_log.txt` on `bravo` AND was received and logged on `alpha` (`[...] Bravo: sec`) - full round-trip, proving `ReadWritePaths` is correctly scoped and the rest of `ProtectHome=read-only` doesn't block the reads (`~/pki`, `~/keyshare`) the app also depends on.
...
- Re-ran the exact same real-mount probe (create default-mode file, `chmod 0600`, `stat`) on `alpha`'s live `~/.securelink/received/` directory - `664` before, `600` after, confirming the fix's mechanism works correctly on the actual production filesystem/mount, not just in theory.
...
- Confirmed `alpha`'s live installed unit (`/etc/systemd/system/securelink-alpha.service`) already carries the Finding #4 sandboxing directives (`ProtectHome`, `RestrictNamespaces`, `MemoryDenyWriteExecute`, etc.) - it did not need reapplying.
