...
| `0x07` | `DESTROY`      | Emergency remote-wipe command, triggered by `/destroy CONFIRM` at either endpoint's compose line (see `session.c`'s `perform_local_destroy()`). Body: none. On receipt, the receiving endpoint immediately and irreversibly wipes ITS OWN local chat state — the persisted message log (`msglog.txt`, in full, deliberately ignoring the `/save` tag that `/clear` respects — this is a genuinely different, more drastic operation than `/clear`), the live on-screen scrollback, and any files saved via a prior `FILE` transfer. No body/nonce is needed because this rides on an already mutually-authenticated (mTLS), replay-protected (per-message HMAC + strictly-increasing `seq_num`, both already required for every message type) session — the sender's identity was already proven during the handshake, and a bare replay of an old `DESTROY` message is rejected the same way a replayed `TEXT_MESSAGE` would be (see Replay protection above). If the peer is unreachable at the moment `/destroy CONFIRM` is issued, the sending endpoint still wipes its OWN state immediately and queues the `DESTROY` message to go out automatically the next time a session connects (reusing the same offline-outbox mechanism `TEXT_MESSAGE` already uses for a disconnected peer). |
